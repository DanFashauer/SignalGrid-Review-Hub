...
1. **Human/lane/tool blockers** (`whoCanUnblock` names the owner, the other lane, or a
   ...
   the owner, a `lane:deliver` message to the other lane, a named setup step). Do not sit
   on them, and do not pretend they are resolved.
2. **Routable blockers** (the monitor named a role): confirm the route is right, then hand
...
3. **GAP blockers** (no role owns this domain): this is the case the owner built you for.
