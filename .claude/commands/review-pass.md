...
- **gate-and-proof-engineer** (only when the diff adds/changes a gate or proof) has Write/Edit and plants defects in the real tree, so it must NOT run in the shared-checkout parallel panel (`dispatching-parallel-agents` excludes shared-state work). Run it AFTER the panel in its own isolated worktree, and **materialize the COMPLETE reviewed tree there first** — a bare `git worktree add --detach` holds only `HEAD`, so apply the step-1 layered diff into it (`git diff HEAD | git -C <tmp> apply`, then copy the untracked matches in) AND run `pnpm install --frozen-lockfile` inside it — a fresh worktree has no `node_modules`, so a proof or gate otherwise stops at `tsx: not found` — before it plants anything; otherwise it falsifies a stale gate, not the staged/unstaged/untracked change under review. For a changed gate this falsification (fail-then-pass on the reviewed change) is REQUIRED before the pass reports green: either plant here on the materialized tree, or STOP and hand the change to the gauntlet — never certify a changed gate unfalsified. A read-only brief is only for when the diff does NOT touch a gate or proof.
