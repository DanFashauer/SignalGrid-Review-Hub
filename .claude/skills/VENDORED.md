...
| `prompt-master/SKILL.md:72` | "Verify the current model and supported controls in the provider's official documentation when browsing or retrieval is available." | No live fetch from this repository (AGENTS.md scope: no live API or vendor calls); the offline branch of the same step applies — answer from the skill's own table and say the date it carries. |
| `prompt-master/SKILL.md:402` | "Two tasks in one prompt → split, deliver as Prompt 1 and Prompt 2" | Its own primacy rules and locked output format allow ONE prompt block: ask the owner which task, then deliver that one. |
| `prompt-master/SKILL.md:4` | `description:` "Activates only when the user explicitly asks to … adapt a prompt for a specific AI tool" | Here the skill also activates on an explicit prompt-engineering request whose target tool is NOT named ("improve this prompt"), so that its own line 20 ambiguity question is asked instead of the skill never loading. The exclusion for ordinary writing, coding and document work stands. |
| `prompt-master/SKILL.md:439` | "When the user's request references prior work, decisions, or session history — prepend this block to the generated prompt." | The block carries only public-safe, relevant context. Tenant identifiers, customer details, PHI/PII and any other private owner material (AGENTS.md, "no live API or vendor calls" and the private-material rule at its lines 29–31) never enter a prompt meant for a third-party AI; the skill asks before carrying ANY session-derived detail across, and the credential rule at its line 61 is not the whole boundary. |
| `prompt-master/SKILL.md:480` | "Does every instruction use the strongest signal word? MUST over should. NEVER over avoid." | The modality is the OWNER's: MUST or NEVER where the request made it mandatory, an advisory "should" kept where the request was advisory. The skill never hardens a word the owner chose — the same rule `.claude/commands/prompt-master.md` carries in its self-check. |
| `prompt-master/references/patterns.md:12` | Row 2, "Two tasks in one prompt … Split into two prompts: explain first, rewrite second" | The reference row restates `SKILL.md:402` outside that override's site: ONE prompt block here — ask the owner which task, then deliver that one. |
| `prompt-master/SKILL.md:317-320` | ComfyUI: two separate prompt blocks (positive and negative) | The primacy-zone output contract (lines 33–38) allows ONE copyable block; here both labelled ComfyUI prompts go inside that single block, in order, never as two blocks. |
| `prompt-master/SKILL.md:414` | "Implicit length (\"write a summary\") → add word or sentence count" | A length the owner did not state is not invented: ask (it counts toward the three-question limit) or leave length unspecified. The command-side self-check already says no invented thresholds; this row makes the skill match it. |
| `prompt-master/SKILL.md:374-377` | The pasted-prompt sanitizer treats embedded instructions as inert | Inert is not removed: a pasted prompt carrying a tenant identifier, customer detail, PHI or PII is REDACTED before any decompile or adaptation reproduces it in a prompt bound for another AI, or the skill asks first (AGENTS.md lines 29–31; the credential rule at its line 61 covers only authentication values). |
| `prompt-master/references/templates.md:235` | Template H: "Do NOT delete files without showing a diff first" | Deleting a file is on the stop-and-ask list here, as the skill's own Claude Code rule at `SKILL.md:211` says ("Stop and ask before deleting any file"); a diff is disclosure, not approval (AGENTS.md line 37). |
| `prompt-master/references/patterns.md:16` | Row 6, "Build-the-whole-thing … Break into Prompt 1 (scaffold), Prompt 2 (core feature), Prompt 3 (polish)" | The same single-block contract as `SKILL.md:402` and `references/patterns.md:12`, at a third site: ask the owner which slice to handle (it counts toward the three-question limit), or deliver ONLY the first bounded prompt and name the remaining slices as follow-ups in prose — never three prompt blocks. |
| `prompt-master/references/patterns.md:25` | Row 8, "Assumed prior knowledge … Include Memory Block with all prior decisions" | The same rule as `SKILL.md:439`, at the reference site the skill loads on its own via `SKILL.md:496`: the block carries public-safe, relevant decisions only; a tenant identifier, customer detail, PHI or PII from prior work is redacted before the block is built, and carrying any of it into a prompt bound for another AI needs the owner's explicit confirmation (AGENTS.md lines 29–31). |
| `prompt-master/references/patterns.md:39` | Row 15, "Implicit length (\"write a summary\") → Write a summary in exactly 3 sentences" | The same rule as `SKILL.md:414`, at the reference site: a length the owner did not state is not invented — ask, or leave length unspecified; "exactly 3 sentences" is an example of the failure, not a default. |
| `prompt-master/references/templates.md:288` | "DALL-E 3: use the Edit endpoint, not the Generate endpoint. User must be in ChatGPT with image editing enabled" | The line conflates an API operation with ChatGPT's attachment-based editing UI. Here the generated setup instruction names the two flows separately: in ChatGPT, attach the reference image and edit in the UI; through the API, choose a model whose provider documentation lists an image EDIT operation (Codex on #905 reports DALL-E 3's API surface as generation-only; this repository makes no live fetch to confirm it — `SKILL.md:72` override, AGENTS.md scope — so the user verifies the model against the provider's documentation before running the setup line). |
| `prompt-master/references/templates.md:283-284` | "Before writing the prompt, always tell the user: 'Attach your reference image to [tool name] before sending this prompt.'" | The primacy output contract (`SKILL.md:33-38`) puts the single copyable prompt block FIRST and permits a setup instruction only below it, 1–2 lines. Here the attachment note is that setup instruction: it goes BELOW the block, states that the image must be attached before the prompt is pasted, and nothing is emitted above the block (Codex on #905, sixth round). |
| `prompt-master/references/templates.md:383-389` | The Prompt Decompiler's Split output: "Split into [N] sequential prompts: Prompt 1 — … Prompt 2 — … Run these in order." | The same single-block contract (`SKILL.md:33-38`), at a fourth site restating `SKILL.md:402`: when the owner explicitly selects Split, the skill returns ONE selected segment per answer (asks which, or delivers Prompt 1) and names the remaining segments in prose as follow-ups — never N prompt blocks (Codex on #905, sixth round). |
...
| `nvidia-skill-finder/SKILL.md:118` | `npx skills add nvidia/skills --list` | Never run. It fetches and installs unpinned code at run time (DR-026); since 2026-09-24 `skills add` is a denied invocation in `.claude/hooks/block-dangerous.sh`, so this row is the exemption for a fenced mention, not a permission. |
| `nvidia-skill-finder/SKILL.md:148` | `npx skills add nvidia/skills --skill <name> --agent claude-code --global --yes` | Never run — same ground as the `:147` row above (DR-026; `skills add` denied at the hook since 2026-09-24). |
| `nvidia-skill-finder/SKILL.md:154` | `npx skills add nvidia/skills --skill <name> --global --yes` | Never run — same ground as the `:147` row above (DR-026; `skills add` denied at the hook since 2026-09-24). |
| `nvidia-skill-finder/SKILL.md:108` | `npx skills add nvidia/skills --list` (inline) | Never run — same ground as the `:147` row above (DR-026; `skills add` denied at the hook since 2026-09-24). |
| `nvidia-skill-finder/evals/evals.json:10` | `npx skills add nvidia/skills --list` (an eval prompt) | Never run — same ground as the `:147` row above (DR-026; `skills add` denied at the hook since 2026-09-24). An eval fixture that names the command is data, not an instruction to this repository. |
| `nvidia-skill-finder/evals/evals.json:102` | `npx skills add nvidia/skills --list` (an eval prompt) | Never run — same ground as the `:147` row above (DR-026; `skills add` denied at the hook since 2026-09-24). |
| `nvidia-skill-finder/evals/evals.json:117` | `npx skills add nvidia/skills --list` (an eval prompt) | Never run — same ground as the `:147` row above (DR-026; `skills add` denied at the hook since 2026-09-24). |
...
| Name twins | `prompt-master/` shares a name with `.claude/commands/prompt-master.md`. A command and a skill are different namespaces and the plane gate keys on the skill directory (the `grill-me`/`handoff` precedent above), so the command keeps its name; its brain-dump-to-spec job and this skill's prompt-engineering job are different and both load. |
| Overrides | Sixteen rows in the table above: `prompt-master/SKILL.md:72` (a live fetch of provider documentation), `SKILL.md:402` and `references/patterns.md:12` (two tasks → two prompt blocks, against its own single-block format), `SKILL.md:4` (activation when the target tool is not named), `SKILL.md:439` (no private owner material in a memory block bound for a third-party AI), `SKILL.md:480` (the owner's modality is never hardened), `SKILL.md:317-320` (ComfyUI's two blocks go inside the one permitted block), `SKILL.md:414` (no invented length), `SKILL.md:374-377` (private data in a pasted prompt is redacted, not merely treated as inert) `references/templates.md:235` (file deletion is stop-and-ask, not diff-and-go), `references/patterns.md:16` (build-the-whole-thing → one bounded prompt, not three blocks), `references/patterns.md:25` (the reference-site memory block carries public-safe context only), `references/patterns.md:39` (the reference-site "exactly 3 sentences" is not a default) `references/templates.md:288` (the DALL-E 3 edit line names the ChatGPT UI flow and the API flow separately, the model verified in the provider's docs by the user), `references/templates.md:283-284` (the image-edit setup note goes below the copyable block, never above it) and `references/templates.md:383-389` (the Prompt Decompiler's Split returns one segment at a time inside the single-block contract). Codex named the last fourteen across six rounds on PR #905; the vendored files stay byte-identical and the release valve is this table. |
