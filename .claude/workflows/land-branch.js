...
//       (`pnpm install --frozen-lockfile --offline`) before anything else — and so
//       does the Pre stage (Codex #1133 P2), when a `preBrief` is given: a Pre stage
//       runs its own gates before the Merge stage ever gets a turn, so the SAME
//       install-if-needed step has to be first there too, not only in Merge.
export const meta = {
  name: 'land-branch',
