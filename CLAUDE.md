...
| `pnpm run check:absence <topic>` | **Before writing "X does not exist."** Probes four differently-shaped ways; exits 1 refuted, 2 inconclusive, 0 corroborated. A word appearing in a disclaimer is NOT the thing existing — that verdict is `inconclusive`, and you read the matches yourself. THREE documents have now claimed a surface was absent when it was sitting in the tree — the third, `docs/company/ICP_EVIDENCE.md` on 2026-08-24, said no competitive surface named OLOID or Imprivata while four anchored briefs and a battlecard had sat in `docs/research/` for two weeks. The rule is not "run it when unsure": run it BEFORE the sentence goes in, because the feeling of being sure is what all three had in common. |
