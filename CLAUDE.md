...
| `bash scripts/mac/install-container-engine-launchd.sh` | **Mac, once, if containers do not come up on their own.** Installs a login agent (`scripts/mac/ensure-container-engine.sh`) that starts whichever container engine is installed — OrbStack / Colima / podman machine / Docker Desktop, detected not assumed (podman before docker, matching `resume-lane.sh`) — and re-checks every 10 min for self-heal. This is the piece `restart: unless-stopped` cannot be: the compose restart policy only resumes containers when the daemon restarts, and nothing launches the engine on a Mac at login unless an agent does. `--compose FILE` also `up -d`s that stack; `--status` / `--uninstall`. |
