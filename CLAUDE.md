...
| `pnpm run hands` | **The channel for a raised hand (DR-054's "who can unblock it").** Everything stuck, grouped by who can clear it: hands agents raised (`pnpm run hand:raise -- --clears owner\|cloud\|mac --what "…" --needs "…"`, or a `raise` op in `lane:deliver batch`; `hand:clear -- <id> --resolution "…"` when done) PLUS stalls nobody raised a hand for — mail unread past 24h, a sim request pending past 48h, a routine past its heartbeat tolerance. A stall past 3× its limit with no hand raised FAILS preflight and CI (`scripts/raised-hands.mjs --check`). Register: `docs/agent/RAISED_HANDS.json`. The owner sees it on one GitHub issue labelled `raised-hands`, refreshed hourly with PRs included; a NEW hand posts a comment so he is notified once. **Answering** a hand: the `raised-hands` skill + `hand-dispatcher` agent route each one by `docs/agent/hand-routing.json` (`hand:take -- <id>` first); a stall kind with no responder, or a responder that does not exist, fails the same gate. The page's health line counts stalls the system found that no agent raised — the monitor on hand-raising itself. |
