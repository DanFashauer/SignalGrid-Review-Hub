...
// first paint; the other monitoring tabs (and the charting library on Overview)
...
function MainLayout() {
  ...
  // docs/EMBEDDED_UX_PRINCIPLE.md the end user never opens a SignalGrid app).
  // Default to Access support — a support lead's window into worker sessions.
    ...
    <div className="flex flex-col h-[100dvh] w-full bg-background text-foreground overflow-hidden">
      <div className="flex-1 overflow-hidden relative">
        <Suspense fallback={<div className="p-4 text-sm text-muted-foreground">Loading…</div>}>
          {activeTab === "access" && <AccessSupport />}
      ...
      <div className="h-14 shrink-0 bg-card border-t border-border flex items-center justify-around pb-safe">
        <TabButton icon={<LifeBuoy size={20} />} label="Support" active={activeTab === "access"} onClick={() => setActiveTab("access")} />
