...
export default function ReviewDashboard() {
  ...
  const completedCount = recommendedActions.filter((a) => completed.has(a.id)).length;
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <div className="w-7 h-7 rounded-md bg-primary/15 border border-primary/40 flex items-center justify-center">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                <rect x="1" y="1" width="5" height="5" rx="1" fill="currentColor" className="text-primary" />
                <rect x="8" y="1" width="5" height="5" rx="1" fill="currentColor" className="text-primary/50" />
                <rect x="1" y="8" width="5" height="5" rx="1" fill="currentColor" className="text-primary/50" />
                <rect x="8" y="8" width="5" height="5" rx="1" fill="currentColor" className="text-primary/30" />
              </svg>
            </div>
            <div className="flex items-baseline gap-2">
              <span className="font-bold text-foreground tracking-tight">SignalGrid</span>
              <span className="text-muted-foreground text-xs hidden sm:inline">Review Workspace</span>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-10 space-y-20">
        ...
        <section className="space-y-6">
          ...
          <div className="border border-primary/30 bg-primary/5 rounded-lg px-5 py-4 max-w-3xl">
            ...
            <p className="text-sm text-foreground leading-relaxed">
              SignalGrid is a <strong className="text-foreground">runtime decision layer</strong> and{" "}
              <strong className="text-foreground">Zero Trust orchestration platform</strong> for shared-device and mobile frontline
              environments — using <span className="text-teal-400">identity</span>,{" "}
              <span className="text-sky-400">device posture</span>,{" "}
              <span className="text-violet-400">session context</span>, and{" "}
              <span className="text-amber-400">operational signals</span> to determine access outcomes{" "}
              <em className="text-foreground">before workflows break</em>. Explicitly not a replacement for IAM, UEM, SIEM, or ITSM.
            </p>
          </div>
            ...
            <div className="md:col-span-2 border border-border bg-card rounded-lg p-5 space-y-4">
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">Composite · 8 dimensions</p>
        ...
        <section id="strengths" className="space-y-6 scroll-mt-20">
          ...
          <div className="space-y-3">
            {strengths.map((item, i) => (
              <ReviewCard key={item.id} item={item} variant="strength" index={i} />
            ))}
          </div>
        </section>
        ...
        <section id="questions" className="space-y-6 scroll-mt-20">
          ...
          <div className="space-y-3">
            {openQuestions.map((item, i) => (
              <ReviewCard key={item.id} item={item} variant="question" index={i} />
            ))}
          </div>
        </section>
        ...
        <section id="actions" className="space-y-6 scroll-mt-20">
          ...
          {completedCount > 0 && (
            <ScoreBar
              ...
              status={completedCount === recommendedActions.length ? "strong" : "developing"}
              size="sm"
            />
          )}
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        <div className="flex items-start gap-2 min-w-0">
                          ...
                          <p className={`text-sm font-semibold leading-tight ${isDone ? "line-through text-muted-foreground" : "text-foreground"}`}>
                            {item.title}
                          </p>
                        </div>
            ...
            {dimensionReviews.map((dim) => {
              ...
              return (
                <div key={dim.key} className="border border-border bg-card rounded-lg overflow-hidden">
                  <button
                    onClick={() => setExpandedDimension(isOpen ? null : dim.key)}
        ...
        <footer className="border-t border-border pt-8 pb-16 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <p className="text-xs font-semibold text-muted-foreground tracking-wider uppercase mb-2">This Workspace Does Not Assert</p>
          ...
          <div className="flex items-center justify-between pt-4 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">SignalGrid Review Workspace · {REVIEW_DATE}</span>
            </div>
...
function ReviewCard({
  ...
  return (
    <div className={`border border-border border-l-2 ${leftAccent} bg-card rounded-lg px-5 py-4 space-y-2`}>
      <div className="flex items-start justify-between gap-3">
        ...
        {item.tag && <div className="shrink-0"><TagBadge tag={item.tag} /></div>}
      </div>
