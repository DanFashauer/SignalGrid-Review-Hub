...
[Get started](https://fleetdm.com/docs) [Controls](https://fleetdm.com/mdm-commands) [Vitals](https://fleetdm.com/vitals) [Reports](https://fleetdm.com/reports) [Policies](https://fleetdm.com/policies) [Software](https://fleetdm.com/software-catalog) [Data tables](https://fleetdm.com/tables)

Search`` `K`

![click to open the table of contents.](https://fleetdm.com/images/icon-hamburger-menu-16x14@2x.png)

GitOps

# GitOps

[Edit page](https://github.com/fleetdm/fleet/edit/main/docs/Configuration/yaml-files.md)

Get started

[Why Fleet](https://fleetdm.com/docs/get-started/why-fleet) [Anatomy](https://fleetdm.com/docs/get-started/anatomy) [Tutorials and guides](https://fleetdm.com/docs/get-started/tutorials-and-guides) [FAQ](https://fleetdm.com/docs/get-started/faq)

Deploy

[Hosting Fleet](https://fleetdm.com/docs/deploy/deploy-fleet) [Single sign-on (SSO)](https://fleetdm.com/docs/deploy/single-sign-on-sso) [Upgrading Fleet](https://fleetdm.com/docs/deploy/upgrading-fleet) [Reference architectures](https://fleetdm.com/docs/deploy/reference-architectures)

Configuration

[Fleet server configuration](https://fleetdm.com/docs/configuration/fleet-server-configuration) [Agent configuration](https://fleetdm.com/docs/configuration/agent-configuration) [GitOps](https://fleetdm.com/docs/configuration/yaml-files)

REST API

[REST API](https://fleetdm.com/docs/rest-api/rest-api)

[Get your license](https://fleetdm.com/new-license) [Contribute](https://fleetdm.com/docs/contributing) [Release notes](https://fleetdm.com/releases) [Support](https://fleetdm.com/support) [Get a demo](https://fleetdm.com/contact) [“Why is Fleet on my computer?”](https://fleetdm.com/better)

On this page:

[General reference](https://fleetdm.com/docs/configuration/yaml-files#general-reference) [labels](https://fleetdm.com/docs/configuration/yaml-files#labels) [policies](https://fleetdm.com/docs/configuration/yaml-files#policies) [reports](https://fleetdm.com/docs/configuration/yaml-files#reports) [agent​\_options](https://fleetdm.com/docs/configuration/yaml-files#agent-options) [controls](https://fleetdm.com/docs/configuration/yaml-files#controls) [software](https://fleetdm.com/docs/configuration/yaml-files#software) [org\_settings and settings](https://fleetdm.com/docs/configuration/yaml-files#org-settings-and-settings)

![A very nice Fleet branded shirt](https://fleetdm.com/images/fleet-shirt-60x55@2x.png)

Request Fleet swag

It's free ![right arrow](https://fleetdm.com/images/arrow-right-red-16x16@2x.png)

On this page

![click to open the table of contents](https://fleetdm.com/images/icon-select-arrows-16x16@2x.png)
...
\
Labels support `path:` (single file) and `paths:` (glob pattern) references. See [`path:` vs `paths:`](https://fleetdm.com/docs/configuration/yaml-files#path-vs-paths-glob-patterns) for details. Filenames must not contain `*`, `?`, `[`, or `{` when using `path:`.\
\
...
\
Policies support `path:` (single file) and `paths:` (glob pattern) references. See [`path:` vs `paths:`](https://fleetdm.com/docs/configuration/yaml-files#path-vs-paths-glob-patterns) for details. Filenames must not contain `*`, `?`, `[`, or `{` when using `path:`.\
\
### Options\
\
...
\
Reports support `path:` (single file) and `paths:` (glob pattern) references. See [`path:` vs `paths:`](https://fleetdm.com/docs/configuration/yaml-files#path-vs-paths-glob-patterns) for details. Filenames must not contain `*`, `?`, `[`, or `{` when using `path:`.\
\
### Options\
\
...
\
- `scripts` is a list of paths to macOS, Windows, or Linux scripts. Supports `path:` (single file) and `paths:` (glob pattern, filtered to `.sh` and `.ps1` files only). Filenames must not contain `*`, `?`, `[`, or `{` when using `path:`. See [`path:` vs `paths:`](https://fleetdm.com/docs/configuration/yaml-files#path-vs-paths-glob-patterns) for details.\
...
\
### apple\_settings and windows\_settings\
\
...
- **`paths:`** accepts a [glob pattern](https://fleetdm.com/docs/configuration/yaml-files#path-vs-paths-glob-patterns) to match multiple files (e.g. `../lib/windows/profiles/*.xml`). Labels and other options specified on a `paths:` entry apply to all matched files.\
\
...
\
Each entry can use either `path:` or `paths:`. Filenames must not contain `*`, `?`, `[`, or `{` when using `path:`. See [`path:` vs `paths:`](https://fleetdm.com/docs/configuration/yaml-files#path-vs-paths-glob-patterns) for glob pattern support.\
\
Use `labels_include_all` to target hosts that have all labels, `labels_include_any` to target hosts that have any label, or `labels_exclude_any` to target hosts that don't have any of the labels. Only one of `labels_include_all`, `labels_include_any`, or `labels_exclude_any` can be specified. If none are specified, all hosts are targeted.\
\
#### android​\_settings.certificates\
\
...
- `certificate_authority_name` is the name of the [certificate authority (CA)](https://fleetdm.com/docs/configuration/yaml-files#certificate-authorities) to issue the certificate from. Currently, only a custom SCEP CA is supported.\
- `subject_name` is the certificate's subject name (SN). Separate subject fields by a "/". For example: "CN= [john@example.com](mailto:john@example.com), O=Acme Inc.".\
\
#### Variables\
\
...
| `$FLEET_VAR_HOST_END_USER_IDP_USERNAME` | macOS, iOS, iPadOS, Windows | Host's IdP username (e.g. " [user@example.com](mailto:user@example.com)"). When this changes, Fleet will automatically resend the profile. |\
| `$FLEET_VAR_HOST_END_USER_IDP_FULL_NAME` | macOS, iOS, iPadOS, Windows | Host's IdP full name. When this changes, Fleet will automatically resend the profile. |\
| `$FLEET_VAR_HOST_END_USER_IDP_USERNAME_LOCAL_PART` | macOS, iOS, iPadOS, Windows | Local part of the email (e.g. john from [john@example.com](mailto:john@example.com)). When this changes, Fleet will automatically resend the profile. |\
...
| `$FLEET_VAR_CUSTOM_SCEP_CHALLENGE_<CA_NAME>` | macOS, iOS, iPadOS, Windows | Fleet-managed one-time challenge password used during SCEP certificate configuration profile deployment. `<CA_NAME>` should be replaced with name of the certificate authority configured in [custom\_scep\_proxy](https://fleetdm.com/docs/configuration/yaml-files#custom-scep-proxy). |\
...
| `$FLEET_VAR_SMALLSTEP_SCEP_CHALLENGE_<CA_NAME>` | macOS, iOS, iPadOS | Fleet-managed one-time Smallstep challenge password used during SCEP certificate configuration profile deployment. `<CA_NAME>` should be replaced with name of the certificate authority configured in [custom\_scep\_proxy](https://fleetdm.com/docs/configuration/yaml-files#custom-scep-proxy). |\
...
\
Currently, you can specify `install_software` in the [`policies` YAML](https://fleetdm.com/docs/configuration/yaml-files#policies) to automatically install software when a host fails a policy.\
\
...
```\
\
#### self\_service, labels, categories, and setup\_experience\
\
...
### app​\_store​\_apps\
\
- `app_store_id` is the ID of the Apple App Store or Android Play Store app. You can find this ID at the end of the app's URL. For example, "Bear - Markdown Notes" URL is " [https://apps.apple.com/us/app/bear-markdown-notes/id1016366447"](https://apps.apple.com/us/app/bear-markdown-notes/id1016366447%22) making the `app_store_id` is "1016366447". Similarly, the URL for "Google Chrome" on Android is " [https://play.google.com/store/apps/details?id=com.android.chrome,"](https://play.google.com/store/apps/details?id=com.android.chrome,%22) so the `app_store_id` is "com.android.chrome."\
...
- `categories` is an array of categories, from [supported categories](https://fleetdm.com/docs/configuration/yaml-files#labels-and-categories).\
\
## org\_settings and settings\
\
...
\
Once the IdP settings are configured, you can use the [`controls.setup_experience.enable_end_user_authentication`](https://fleetdm.com/docs/configuration/yaml-files#setup-experience) key to control the end user experience during ADE.\
\
- `idp_name` is the human-friendly name for the identity provider that will provide single sign-on authentication (default: `""`).\
- `entity_id` is the entity ID: a Uniform Resource Identifier (URI) that you use to identify Fleet when configuring the identity provider. It must exactly match the Entity ID field used in identity provider configuration (default: `""`).\
- `metadata` is the metadata (in XML format) provided by the identity provider. (default: `""`)\
...
\
For possible options, see the parameters for the [smtp\_settings object in the API](https://fleetdm.com/docs/rest-api/rest-api#smtp-settings).\
\
Can only be configured for "All fleets" (`org_settings`).\
\
##### Example\
...
\
![An arrow pointing up](https://fleetdm.com/images/icon-arrow-up-12x13@2x.png)Back to top\
\
