...
- [ ] **Resolve every VENDORED.md Overrides row's cited `path:line` against the real file (gates, MEDIUM; owner: devex-tooling-engineer).** `parseOverrides` is already exported from `scripts/check-skill-instruction-conflicts.mjs:266`; ~20 lines resolving each row's backticked `path:line` under `.claude/skills/` would make a fabricated citation fatal. Without it nothing fails: the original unlazy proposal cited `SECURITY.md:122` and `:109` in a 72-line file (`wc -l` = 72) and `src/telemetry.ts:84` for a symbol at `:86`, and `check-skill-instruction-conflicts.mjs:99-101` trusts the table for "existence and shape" only while `:100` says a STALE row is "Deliberately NOT reported" — so the one table both intake proposals leaned on for their safety argument is the one table nothing drift-checks. Same fossil class `checkCitations` already guards for PRODUCERS in `check-gitignore-producers.mjs`. Ships with a `--self-test` planting a row whose line number is past EOF.
- [ ] **Byte ledger for the 100 vendored skill directories, built on `git ls-tree` tree oids (gates, LOW; owner: devex-tooling-engineer).** Append a line of prose to `.claude/skills/watch/SKILL.md` and nothing in the tree goes red — verified three ways: `check-skill-plane-conformance.mjs` reads only frontmatter name/description ("117 skill(s), 13 agent(s) walked … passed"), `check-skill-instruction-conflicts.mjs` judges only hook-denied command text, and `grep -rln createHash scripts/*.mjs scripts/lib/*.mjs` returns five files, none of which reads `.claude/skills`. `check-agent-roster.mjs:153-163` already refuses agent drift, but only because that vendor tree is in-repo and can be `cmp`'d. Build it as one `git ls-tree HEAD .claude/skills/` read (a content-addressed oid per directory, one command, no hash code, tracked-paths-only for free — NOT a re-implemented sha256 walk, and nothing is lifted from vercel's upstream's src/local-lock.ts (vercel-labs/skills, not in this tree), so no licence basis is owed), rows derived from `scripts/lib/skill-plane.mjs` `trackedSkillDirs()`/`splitSkillPlane()` which already throw on an unparseable table, `--write` as the only writer, and each row carrying its upstream pin so a changed oid fails UNLESS that upstream's VENDORED.md section changed in the same commit. Header must say in its first sentence that it certifies "unchanged since minted", never upstream provenance and never tamper-resistance (`--write` is in-tree and unprivileged). Priority is LOW, and lower than when it was proposed: its strongest argument was that vendoring unlazy would put third-party executable `.mjs` under `.claude/skills` for the first time, and the adopted design puts the linter in `third_party/` instead. Register it in preflight AND its CI twin in the same change — `check-preflight-ci-parity.mjs` goes red on a half-registration, and `check-gate-census.mjs:63` only sees gates that have a `package.json` script, so a deferred registration ships a gate no lane runs.
- [ ] **Refuse a tracked ledger carrying `CHECK:` gate lines outside `third_party/unlazy/templates/` (gates, LOW; owner: security-engineer).** A `CHECK:` is a shell string, and the vendoring brings ledger-shaped files with `CHECK:` lines into the tree (`templates/gates-node.md:6,11,16,21`, `templates/gates-leaf.md:8,13`). Nothing stops a `GATES.md` from being committed to a branch, arriving in a PR body, or being written by a lens and approved against this tree by a later session — the inherited-ledger shape the orchestrator skill forbids in prose. Extend an existing gate to fail on any tracked file outside that templates directory carrying a `CHECK:` gate line. Lower priority under the adopted design than as proposed: with the executor un-vendored, nothing IN this tree can execute such a ledger, so the risk is a future re-vendor, not today — but that is precisely what stops the exclusion list from being a "NOT taken" cell in a document.
- [ ] **Lint arm refusing an `EXPECT:` a failing run can also print (gates, LOW; owner: devex-tooling-engineer).** The vacuous-pass shape: a ledger whose `EXPECT:` token appears in a FAILING run's output certifies green from a failure, and upstream's linter only WARNs on a manual gate — measured, `gate-lint` flags a manual gate as `WARN [manual-gate]` and refuses nothing about EXPECT's selectivity. The orchestrator skill will carry the authoring rule (prove the token against a known-failing control once); this is the mechanical half, and it belongs in a first-party wrapper or the doctrine's own check, never as an edit to the vendored byte-identical `gate-lint.mjs`. Without it the rule is remembered, not gated — the same class as N4.
- [ ] **Append the decision record for the unlazy intake and the two hook denials, numbered only after PRs #1019 and #1031 have landed (records, MEDIUM; owner: records-archivist).** One record, not two — the three `nvidia-skill-finder` Overrides rows are what turn `skills add`'s refusal from a table row into enforcement, and they are the same decision as vendoring the linter without its executor. It carries three things: (a) the executor is refused, and if ever revisited the subject is a mechanical ledger-provenance check, never a paragraph; (b) two commands (`skills add`, `install-hooks`) are now refused at `.claude/hooks/block-dangerous.sh`, which changes what a session may run; (c) nothing changes how green is certified — the orchestrator re-runs each `CHECK:` through Bash and quotes output. The number is read from the tail of `docs/DECISION_RECORDS.md` at that moment (DR-054 and DR-056 sit in the open PRs), never typed early. From the 2026-09-24 intake (`docs/agent/RESOURCE_INTAKE.md`).
