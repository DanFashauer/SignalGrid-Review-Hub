...
- [ ] **A by-reference operator procedure for YouTube URLs that `/watch` cannot download (video intake, LOW).** On 2026-09-12 `yt-dlp` got HTTP 403 from YouTube on a full-video `/watch` run and only the captions path succeeded; a 2026-09-20 owner-shared clip's recipe for that gap is a Gemini key from Google AI Studio, which reads a YouTube URL without downloading it. That is an upload of the URL — and, for owner media, the file — to a third party, so it is the same per-machine owner decision as a Whisper key (DR-040, DR-029), and a live vendor API call has no place in this public repository (AGENTS.md scope: no live API calls). If the owner takes it, the deliverable here is ONE paragraph in `.claude/skills/video-intake/SKILL.md` naming the operator-side procedure — key and run both outside the tree, the transcript arriving as a plain file the skill's Step 2 already reads, a `RESOURCE_INTAKE` row per use — NOT a script in this tree, NOT a flag on `transcribe-local.py`, NOT a gate that needs the network. Never a default, never on the Mac tick, never a transcript treated as a fixture (video-intake rule 3). The check that fails without it, offline: the skill's "What this skill does not do" names the boundary but no documented procedure exists for a URL that cannot be fetched, so such a clip is logged with no transcript at all. devex-tooling-engineer.
