...
- [x] **Add `docs/agent/objective.json` to `SAFETY_MACHINERY` in `scripts/check-owner-gated-surfaces.mjs` — the second belt on the owner attestation (agent plane, LOW; owner: agent-platform-engineer).** DONE 2026-09-25 (cloud): one `{rule, re}` line (`the declared objective (DR-056 attestation pointer)`) plus two self-test lines — `objective.json` classifies owner-gated, and the tick-written `objective-state.json` does NOT (the Mac rewrites it unattended). Check: `node scripts/check-owner-gated-surfaces.mjs --self-test` prints N/N (28/28 on 2026-09-25; 43/43 after the 2026-09-26 review-fix pass added OWNER_RESERVED rules for the launch-claims gate/publication boundary/launch-profile machinery, a SAFETY_MACHINERY-ordering case, and CLI-level --classify-branch coverage for renames, non-ASCII paths and bad refs; 49/49 after the Codex #1133 fix pass added OWNER_RESERVED rules for check-launch-claims.mjs's own two ceiling files and SAFETY_MACHINERY rules for `.claude/workflows/`, `.claude/hooks/` and `.claude/settings.json`; 53/53 after the #1133 review-nit pass added SAFETY_MACHINERY rules (plus a negative self-test) for `.claude/skills/landing-under-dr-037/`, `.claude/skills/orchestrator-over-workers/` and `.githooks/`; 63/63 after Codex's round-2 pass on #1133 added OWNER_RESERVED rules for the repository instruction files (root `AGENTS.md`, `CLAUDE.md`) and for `scripts/lib/ratchet-read.mjs` (the launch-claims gate's own imported ratchet-read helper — previously only SAFETY_MACHINERY via the blanket `scripts/**` rule, one tier below the gate script that imports it), a SAFETY_MACHINERY rule for the root `package.json` (`pnpm-workspace.yaml` already had one, shared with `pnpm-lock.yaml`), a negative self-test proving a per-package manifest outside `lib/**`/`scripts/**` stays autonomous, and a self-test that RE-DERIVES every owner-reserved gate script's own relative imports at test time — reading `check-launch-claims.mjs`, `check-publication-boundary.mjs`, `publication-boundary.mjs` and `check-launch-profile.mjs` from disk, resolving their imports one level deep, and asserting each resolved path classifies `OWNER_RESERVED` — so a future import a gate script adds cannot silently escape the manifest). The criterion ids, scopes and attestation token already live in `scripts/objective-loop.mjs`, so the json alone cannot reach `goal_met`; this makes the file itself owner-gated so a changed `attestedIn` pointer is reviewed as safety machinery, not as a doc. One `{rule, re}` line plus its self-test line; deferred from the DR-056 PR only because #1019 also edits that file's neighbours. From the 2026-09-24 objective-loop build (critic finding, taken).
...
- [x] **Stamp every open Global-backlog row in `docs/COMPANY_BUILD_PLAN.md` with a `re-measured YYYY-MM-DD`, and make `scripts/objective-loop.mjs` refuse to rank a row whose stamp is missing or older than 14 days (agent plane, MEDIUM; owner: program-manager → devex-tooling-engineer).** DONE 2026-09-26 (cloud) for the refusal; the stamping is the measuring itself and lands row by row. `rowMeasuredAt` reads the latest `re-measured YYYY-MM-DD` in a row (quoted and code spans removed, as `statusText` does for status markers); `rank()` refuses an open row with no stamp, a future one, or one older than `MEASURE_WINDOW_DAYS` (`14`) BEFORE resolving its executor, lists it in the state's new `unmeasured[]` with the reason, and raises ONE `plan-rows-unmeasured` escalation for the cloud naming every refused id. `finalize` and `--check` count the refused rows as a named reason (not a vacuous plan); `--check` REPORTS the count and ids rather than failing — with `108` of `109` open rows unstamped on the day this landed, a fatal gate would have been switched off the same hour, and the row-by-row measuring is the work the escalation now assigns. Only row 12 carried a stamp within the window on landing day, so the next derivation ranks it alone. Check: `node scripts/objective-loop.mjs --self-test` (the `stamp:` and `check:` cases — a 15-day stamp, an unstamped row, a quoted stamp and a future stamp are each refused with a named reason; a state whose only work is refused rows passes with the refusal REPORTED) and `node scripts/objective-loop.mjs --json` (`report.unmeasuredCount`). Four of the loop's top-ranked rows (5, 6, 8, 9) were found on 2026-09-25 to have shipped 23–36 days earlier with the row still reading open: the loop was ranking finished work, and the tick's `tasks[0..2]` steered the forward-build trigger at it. A stale row is an unknown input, and unknown tightens (rank nothing from it and name the row) rather than loosens. Check that would fail today: an `objective-loop.mjs --self-test` case where a row stamped 15 days ago is excluded with a named reason, and where an unstamped open row fails `--check`.
...
### DR-060 — the brain delegates; the cycle learns (2026-09-26, cloud lane)

- [x] **The plan-row measurement tranche is a scripted workflow: tiered workers measure, stamp, run the FULL preflight in the worktree, then push; no hand-picked gate subset stands in for preflight (agent plane, MEDIUM; owner: devex-tooling-engineer).** Lesson L2 in `docs/agent/LESSONS.md`: tranche PRs went out after seven hand-picked doc gates, the list left out cross-doc banner parity, and CI's Mac job 108335885542 failed at `Preflight FAILED at: Cross-doc banner parity`. The workflow's push stage must refuse a head that has no `PREFLIGHT_EXIT=0` sentinel written for that sha in its own worktree, and its self-test plants a missing and a non-zero sentinel and asserts the refusal. Without it the check that fails is CI's, after the push — which is what happened. DONE 2026-09-26 (cloud, corrected 2026-09-26 review pass): `.claude/workflows/land-branch.js`, a tracked saved workflow (Workflow tool, name `land-branch`), makes this structural rather than a rule someone re-reads — but the refusal is enforced by the SCRIPT, not by worker prose: the Chain phase's push agent is a separate stage the script dispatches ONLY after computing `canPush()` (`scripts/lib/land-branch-gate.mjs`) over the Chain-run worker's report, requiring `preflightExit===0 && breadthExit===0`, the run's own `headSha` to equal the merge head, AND both sentinel log lines to EQUAL exactly `PREFLIGHT_EXIT 0 <expected-head>`/`BREADTH_EXIT 0 <expected-head>` (closing the earlier gap where a tag-scoped log could still hold a PREVIOUS run's sentinel for a different sha). The self-test the row asks for is `node scripts/lib/land-branch-gate.mjs --self-test`, which plants a missing sentinel, a non-zero sentinel, and a stale sentinel at the wrong sha, asserts refusal in each case, and fails when the workflow's inlined mirror of `canPush` drifts from the module (19/19 on 2026-09-26). Check that would fail without it: `node scripts/lib/land-branch-gate.mjs --self-test` and `node scripts/check-lessons.mjs` (L2 requires a checkable, tracked landing). **Updated 2026-09-26 (Codex P1/P2 on #1130):** the sentinel check had drifted to a "literally contain"/`.includes(expectedHead)` substring test, which a line like `PREFLIGHT_EXIT 0 <stale> unrelated=<expected>` passed; `canPush()` now requires the WHOLE line to equal the exact string, `verify()`'s line parser is anchored (`^<LABEL>_EXIT (-?\d+) ([0-9a-f]{40})$`) and refuses a well-formed sentinel naming the wrong head, and the push worker's one-liner runs fully under `&&` (was `tee ...; grep ...`, so a failed `cd` fell through to `grep` against a stale `/tmp` file from an earlier run) with its output file moved under `<scratch>/<tag>-verify.out`, never `/tmp`.
- [x] **MCP roster: one document naming which MCP servers and APIs each lane and skill may call and for what, with the repo's own server's tool count derived, not typed (agent plane, MEDIUM; owner: agent-platform-engineer).** DR-060 rule 3's first slice. `docs/agent/mcp-roster.json` today dispositions third-party servers only; it does not say which lane or skill may call which server, and nothing counts `signalgrid-mcp`'s tools (`grep -c 'registerTool(' artifacts/mcp-server/src/index.ts` → 16 on 2026-09-26). Extend the roster with per-lane and per-skill grants and a gate with a `--self-test` that fails when a skill names an `mcp__` server the roster does not grant, or when the roster's stated tool count for `signalgrid-mcp` differs from the `registerTool(` count in `artifacts/mcp-server/src/index.ts`. DONE 2026-09-26 (cloud): `docs/agent/mcp-roster.json` gained a `signalgrid-mcp` `servers[]` entry (16 tools, `toolNames` in source order, 7 read-only / 1 mutating / 8 unannotated), a top-level `external` array for github/firecrawl/neural-memory, and `grants.lanes`/`grants.skills`/`ungranted` naming which lane and which first-party skill may call which server and for what — lane grants sourced from `docs/MCP_AND_SKILLS_LANE_PARITY.md`, skill grants from the named skill docs themselves. `scripts/check-mcp-roster.mjs` derives the tool count and name list straight from `artifacts/mcp-server/src/index.ts`'s `registerTool(` calls (16 on 2026-09-26, matching the roster) and cross-checks that derivation against a plain `registerTool(` count so a call written in an unrecognized shape fails instead of vanishing, checks every granted server id exists, every `grants.skills` key is a real first-party skill directory, and fails naming the file/server/missing-grant when a first-party skill doc names an `mcp__<server>__` call the roster does not grant. Registered in `scripts/preflight.mjs` and `.github/workflows/review-hub-ci.yml` right after the land-branch push-gate entry. `--self-test`: 15/15 (a wrong tool count, a missing toolName, an extra toolName, a grant to an unknown server, a grant to an evaluated-not-adopted or deferred server, a skill doc naming an ungranted `mcp__ghost__x` call, a registerTool( call in an unrecognized shape, a roster missing `servers[]`/`grants{}`, zero `registerTool(...)` matches, zero first-party skill directories, a `grants.skills` key that is not a first-party skill directory, a `grants.skills` entry naming an unknown server, and an unparseable roster all FAIL; one clean synthetic roster PASSes). **Updated 2026-09-26 (Codex #1126/#1127 review fixes):** `--self-test` is now 28/28 — added `grants.mentions` structural checks (D1-style: a `grants.lanes`/`grants.skills`-shaped required-map check, ordered `toolNames`, `for`/`why`/`source` purpose fields, and disposition gating all extended to `grants.mentions`), a check that a `grants.mentions` key is itself a real first-party skill directory (it was previously unchecked), a fix so a `grants.mentions` entry no longer clears an actual `mcp__<server>__` call token (only `grants.skills` can — a mention means "named as precedent, not called"; the D5 loop now tracks called-names and mentioned-names as separate sets), and a non-array `grants.skills`/`grants.lanes`/`grants.mentions` entry now reports a named problem string instead of throwing an uncaught `TypeError`. `node scripts/check-mcp-roster.mjs` against the real roster: 6 servers (+3 external), signalgrid-mcp 16/16 tools derived, 12 lane grants, 6 skill grants over 18 first-party skills, 11 mentions, 0 problems.
- [x] **land-branch.js PR-body stage should run on Haiku, not Sonnet, per DR-060's own stage table — SUPERSEDED in the other direction (agent plane, owner: agent-platform-engineer).** Codex summary finding 6 on #1126/#1127 asked that the `body` stage move to `model: 'haiku'` to match the stage table. Instead the table was wrong: two Haiku PR-body drafts (the #1119 landing and the 07:20Z record) fabricated a check-run id, a Jekyll cause and file attributions, so `docs/agent/LESSONS.md` row L16 keeps the workflow's `body` stage on Sonnet and moves the stage table's PR-body assignment there too (`.claude/skills/orchestrator-over-workers/SKILL.md`, the row now reads "PR bodies (Sonnet since L16 — Haiku fabricated a check-run id and file attributions twice on 2026-09-26)"). Finding 6 is closed by L16, not by flipping `land-branch.js`. Check: `node scripts/check-lessons.mjs` (L16 present, landed) and the stage table row in `.claude/skills/orchestrator-over-workers/SKILL.md`.
- [x] **land-branch.js's "Owner decision needed" text should be derived from the changed paths, not a three-way `klass` ternary (agent plane, MEDIUM; owner: devex-tooling-engineer).** Codex summary finding 7 (P1) on #1126/#1127: the PR-body stage's owner-decision paragraph is chosen by `klass === 'SAFETY_MACHINERY' ? ... : klass === 'DECISION_PATH' ? ... : ...` — a caller-supplied string the workflow never checks against the diff, so a caller that mis-classifies its own branch (or a stacked branch whose paths shifted since `klass` was chosen) prints an owner-decision paragraph that doesn't match what actually changed. `scripts/check-owner-gated-surfaces.mjs` already computes SAFETY_MACHINERY/DECISION_PATH classification from paths for its own gate; the PR-body stage should call the same classifier over `git diff --stat` against the merge base instead of trusting `klass` as typed. Verified but deferred out of the 2026-09-26 hardening pass (recorded only in that commit's message until now). Check that would fail today: a synthetic run where `klass` says `DECISION_PATH` but the diff touches only `docs/`, asserting the printed owner-decision text still says "None" rather than the DECISION_PATH boilerplate. DONE 2026-09-26 (cloud): `scripts/check-owner-gated-surfaces.mjs` gains `--classify-branch <base-ref>` (diffs the merge-base of `<base-ref>` and `HEAD`, prints exactly `KLASS <klass> files=<n> matched=<m>` or, fail-closed on a git failure or an empty diff, `KLASS ERROR <reason>` at exit 2) and the pure `mostRestrictive()` helper it uses, ranking `OWNER_RESERVED > DECISION_PATH > SAFETY_MACHINERY > other` (self-test 31/31, including the ordering cases the row asked for: `scripts/` + `lib/signalgrid-core` → DECISION_PATH, plus `docs/LAUNCH_PROFILE.md` → OWNER_RESERVED, docs-only → other). `scripts/lib/land-branch-gate.mjs` gains `resolveKlass(callerKlass, derivedLine)` (parses that line; the DERIVED class always wins over the caller's, even to escalate a caller's `other` up to `OWNER_RESERVED`; an unparsable line, a `KLASS ERROR` line, or `files=0` refuses) and `ownerDecisionText(klass)` (the three existing paragraphs lifted verbatim plus a new `OWNER_RESERVED` branch; an unrecognised klass throws rather than guessing), both mirrored byte-for-byte into `.claude/workflows/land-branch.js` next to `canPush`'s mirror and checked by the same drift test (self-test 45/45 — 28/28 originally, plus the Codex #1133 P1 fix pass, plus the #1133 review-nit pass below, including the row's own synthetic case: caller `DECISION_PATH` + derived `KLASS other files=2 matched=0` → resolved `other`, text starts with "None"). The Merge stage now runs the classifier after the Alpha merge, returns its line as `klassLine`, and the script resolves it before the PR-body stage runs — using the DERIVED class everywhere `klass` used to be trusted, logging an override, and logging (never silently merging) when the diff resolves to `OWNER_RESERVED`. **Updated 2026-09-26 (Codex #1133 P1):** `klassLine` was still worker-reported prose with nothing re-checking it against the diff at PUSH time — a fabricated but validly-shaped line (`KLASS other files=3 matched=0`) would resolve cleanly and steer the push. `scripts/lib/land-branch-gate.mjs`'s `verify()` now takes an optional `klass` and, once the sentinel/head/ref checks already pass, re-runs `node scripts/check-owner-gated-surfaces.mjs --classify-branch origin/SignalGrid_Alpha` itself in `<worktree>` and refuses (`derived class <X> !== resolved class <Y>`, or `classifier did not print a KLASS line: <raw>`) unless the classifier's own answer equals `klass`; the `--verify` CLI validates `--klass` against the four known classes and, when given, prints `land-branch-gate --verify PASS: head <sha> klass <klass> …` (unchanged shape when `--klass` is omitted). The push worker's one-liner now passes `--klass <klass>` (the resolved class) and greps the exact `klass <klass>`-qualified PASS prefix, so the push is bound to the classifier's own answer on the exact verified head, never to a worker's account of it. **Updated 2026-09-26 (review nits on #1133):** `classifyBranchOutput()` now reads the classifier via `git show <baseRef>:scripts/check-owner-gated-surfaces.mjs` into a throwaway temp file instead of the worktree's own working copy, so a branch that rewrites its own classifier (e.g. emptying every manifest rule) is still graded by the BASE ref's real one — self-tested by committing exactly that rewrite on a branch and asserting `--klass SAFETY_MACHINERY` still passes while `--klass other` refuses. A non-zero classifier exit (a `git show` failure or the classifier process itself exiting non-zero) is now always a refusal that names the exit code and first output line, never stdout handed through as a candidate `KLASS` line. `runVerifyCli` now refuses a bare `--klass` (or a value outside the four known classes) with a `REFUSED` reason and exit 1 instead of silently falling back to the pre-`--klass` PASS line. Both `.claude/workflows/land-branch.js` install lines (Pre stage step 0, Merge stage step 0) are now `cd ${worktree} && { test -x scripts/node_modules/.bin/tsx || pnpm install --frozen-lockfile --offline; }` so a failed `cd` can never install into the wrong directory. `scripts/check-owner-gated-surfaces.mjs`'s SAFETY_MACHINERY manifest gains `.claude/skills/landing-under-dr-037/`, `.claude/skills/orchestrator-over-workers/` and `.githooks/` (these govern how the lane lands and what the pre-push hook enforces), with a negative self-test proving an unrelated skill dir stays autonomous. **Updated 2026-09-26 (Codex round 2 on #1133):** the Merge stage's step 0 only installed the worktree BEFORE the Alpha merge — a merge that itself changes the lockfile left every generator and gate in the same stage running against whatever `node_modules` predated it. A new step 3 (`cd ${worktree} && pnpm install --frozen-lockfile`, its last line reported in `gateResults`) now runs right after the merge commits and before the coverage/manifest regenerators and quick gates, renumbering the old steps 3-5 to 4-6. The Chain stage's detached job installs a THIRD time, from the lockfile, inside the existing subshell, chained with `&&` onto `node scripts/preflight.mjs` (`pnpm install --frozen-lockfile > <tag>-install.log 2>&1 && node scripts/preflight.mjs > <tag>-pf.log 2>&1; echo "PREFLIGHT_EXIT $? $(git rev-parse HEAD)" >> <tag>-pf.log; …`), so the preflight/breadth sentinel certifies the tree CI actually installs; an install failure short-circuits the `&&` and the immediately-following `echo` captures the install's own non-zero `$?`, still producing a well-formed `PREFLIGHT_EXIT <n> <sha>` line that `canPush()` refuses on exactly as it would a real preflight failure — the lock trap and both sentinel formats are otherwise unchanged. The job-started `ps` check (and its worker instruction) now matches EITHER the install or `node scripts/preflight.mjs`, since the install can still be running at the 2-second mark. All three `pnpm install` call sites (Pre step 0, Merge step 0, Merge step 3) dropped `--offline`, and so did `docs/agent/LESSONS.md`'s L17 landing note and this file's own two mentions above — the flag turned a genuinely stale lockfile's clear `ERR_PNPM_OUTDATED_LOCKFILE` into a confusing "not found in offline mode" for no benefit where the registry is reachable. `scripts/check-owner-gated-surfaces.mjs` also gains OWNER_RESERVED rules for root `AGENTS.md`/`CLAUDE.md` and for `scripts/lib/ratchet-read.mjs`, and a SAFETY_MACHINERY rule for the root `package.json` (self-test 63/63, see the row above); `scripts/lib/land-branch-gate.mjs`'s own self-test is unchanged at 45/45 (`canPush`/`resolveKlass`/`ownerDecisionText` and their mirrors were not touched by this pass — only the prompt text around them).
- [ ] **The OIDC middleware test binds ephemeral ports, as #1106 did for `api.test.mjs` (api-server tests, MEDIUM; owner: devex-tooling-engineer).** Lesson L1's root cause in `docs/agent/LESSONS.md`: `artifacts/api-server/test/oidc.test.mjs` still fixes `JWKS_PORT = 5399`, `PORT = 5398` and `ABSENT_PORT = 5397` (and derives `PORT + 40`), so an api-server on any of those ports from ANY tree on the host fails preflight with `EADDRINUSE 127.0.0.1:5399`. Preflight's orphan reap covers only its own tree's orphans and cannot fix a cross-tree collision. Move each to the `freePort()` shape `api.test.mjs` uses; the check that fails today is running the OIDC test with a planted listener on 127.0.0.1:5399 and asserting it passes.
- [x] **CI-side check that reads the last conclusion of every non-gating mainline workflow (Pages, CodeQL, Supply Chain, ...) and REPORTS a red streak longer than N pushes (CI plane, MEDIUM; owner: devex-tooling-engineer).** Lesson L8 in `docs/agent/LESSONS.md`: the GitHub Pages "pages build and deployment" workflow concluded failure on every push to SignalGrid_Alpha for 34 days (8dbea78d, 2026-08-23, through 2026-09-26) because a vendored skill file's JSX code sample breaks Jekyll's Liquid parser, and a 34-day red Pages build went unseen because nothing read it — no gate reads a non-gating workflow's own run conclusion. Check that would fail today: query each named workflow's last N run conclusions (GitHub API or `gh run list`) and assert nothing reports N consecutive failures; run it against the Pages workflow's real history and it should have failed from run 301 onward. DONE 2026-09-26 (cloud, branch `claude/check-mainline-workflow-streaks`): `scripts/check-mainline-workflow-streaks.mjs` lists the repo's workflows through the Actions API, reads the last 10 completed runs on SignalGrid_Alpha of each of the thirteen workflows that run there unwatched by any other gate (GitHub's dynamic Pages workflow, the gating workflow itself — its branch filter already restricts this read to SignalGrid_Alpha push runs — plus eleven more files), and REPORTS every one whose last 3 judged runs are red, with the streak length (≥ when it fills the window), its earliest run's time and the run URL. Report-only on a streak by design on day one, not the fail this row first proposed. A workflow file in neither `WATCHED` nor `NOT_WATCHED` is a tree defect and stays fatal everywhere; every other own error — an HTTP error (a proxy token that lacks `actions: read` returns 401, same as any other unreachable API), the dynamic Pages path missing from the Actions API listing, a malformed payload, or no token in CI — is FATAL in CI (the workflow step hands over a real token, so an unreachable API there means the check could not run where it must) and REPORTED, exit 0, outside it (a local or container `GH_TOKEN`/`GITHUB_TOKEN` is often a git-proxy credential with no API scope at all, and failing a developer's preflight over that is how a gate gets switched off — same rule as `check-ci-liveness.mjs`'s header, "FATAL IN CI, REPORTED LOCALLY"). A watched workflow FILE absent from the listing — new on this branch, or renamed — is skipped and named, never thrown. An unknown run conclusion (anything outside success/neutral and the red/no-verdict sets) still ends a streak but is named on the row rather than silently treated green. Wired in `scripts/preflight.mjs` (prints REPORTED off CI when the API is unreachable, or SKIPPED with no GITHUB_TOKEN at all, both classified as self-skips) and in `.github/workflows/review-hub-ci.yml`'s validation job with the workflow token. Check: `node scripts/check-mainline-workflow-streaks.mjs --self-test` (32/32, including the synthetic Pages payload read as a streak of 10 or more through `--fixture`, and an end-to-end 401 through `runLive()` proving REPORTED/exit-0 off CI vs FATAL/exit-1 in CI); the real Pages history is read on the first CI run.
