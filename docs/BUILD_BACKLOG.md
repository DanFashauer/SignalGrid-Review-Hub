...
- [x] **The OIDC middleware test binds ephemeral ports, as #1106 did for `api.test.mjs` (api-server tests, MEDIUM; owner: devex-tooling-engineer).** DONE 2026-09-26 (cloud): `artifacts/api-server/test/oidc.test.mjs` (`JWKS_PORT`, `PORT`, `ABSENT_PORT`, and each broken-config-loop `brokenPort` — no more `PORT + 40` offset arithmetic), `artifacts/api-server/test/load.test.mjs` (`PORT`, `LIMIT_PORT`) and `scripts/src/observability-proof.ts` (`PORT`) all bind OS-assigned ports via the `freePort()` shape #1106 introduced, so none of them can collide with a fixed port bound by another tree or tick on the same host again. Measured: `node --test artifacts/api-server/test/oidc.test.mjs` → 21/21 assertions passed; `node --test artifacts/api-server/test/load.test.mjs` → summary=pass (18/18); `pnpm run proof:observability` → 10/10 assertions passed. `grep -n '53[0-9][0-9]' <file>` shows no hits in any of the three files. The row's own falsifying check ran 2026-09-26 on Linux: a listener was planted on every fixed port the branch removed at once — 5320, 5321, 5397, 5398, 5399, 5438, 5439, 5388 (the last two are the old `PORT + 40`/`PORT + 41` offsets, since `PORT` itself was 5398) — confirmed held via `lsof -iTCP -sTCP:LISTEN`, then all three suites were run against that occupied range: `node --test artifacts/api-server/test/oidc.test.mjs` → `OIDC middleware test: 21/21 assertions passed`, exit 0; `node --test artifacts/api-server/test/load.test.mjs` → `summary=pass (18/18)`, exit 0; `pnpm run proof:observability` → `Observability proof: 10/10 assertions passed`, exit 0. `freePort()`'s OS-assigned ports never collided with the held ones. The planted listener was then killed and the ports confirmed free again.
- [ ] **CI-side check that reads the last conclusion of every non-gating mainline workflow (Pages, CodeQL, Supply Chain, ...) and REPORTS a red streak longer than N pushes (CI plane, MEDIUM; owner: devex-tooling-engineer).** Lesson L8 in `docs/agent/LESSONS.md`: the GitHub Pages "pages build and deployment" workflow concluded failure on every push to SignalGrid_Alpha for 34 days (8dbea78d, 2026-08-23, through 2026-09-26) because a vendored skill file's JSX code sample breaks Jekyll's Liquid parser, and a 34-day red Pages build went unseen because nothing read it — no gate reads a non-gating workflow's own run conclusion. Check that would fail today: query each named workflow's last N run conclusions (GitHub API or `gh run list`) and assert nothing reports N consecutive failures; run it against the Pages workflow's real history and it should have failed from run 301 onward.
