...
- [x] **Align the new 27.0 `mdm.*` status items to the custody model (DDM connector, HIGH).**
      *(Row text discovered 2026-09-18 on `origin/claude/intake-2026-09-18`, which is not
      merged; it is transcribed here in full so the record sits where the work does. What
      27.0 ADDS was unmodelled: `mdm.is-return-to-service`, `mdm.enrollment-type`,
...
- [x] **Hold the DDM/macOS-posture schema pins against Apple's YAML (gate, MEDIUM).**
      *(Same provenance as the row above.)* **DONE 2026-09-18** — the ten status items the
