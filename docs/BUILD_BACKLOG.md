...
- [x] **Non-demo core constructor.** The served API builds `SignalGridCore.demo()`; it never wires the real Graph connector in `lib/`. Largest gap to a real customer. (gap `non-demo-core-constructor`.) Lane: principal-engineer. **DONE 2026-09-18** — PR #854 (`SignalGridCore.fromEstate()`, selected by the `SIGNALGRID_CORE` variable). Checked against GitHub, not memory: PR #854 is currently **open** (`merged: false`), targeting `SignalGrid_Alpha`; its description carries "Merge note: Product change: **owner-merged** (DR-037)" — that PR is open and owner-merged; say so, and do not read "open" as "not landing" or "merged" as "already in the tree" — as of this check the branch this row lives on does not carry `5de9bcd1`/`SignalGridCore.fromEstate` yet.
