...
- [x] **iOS ExpiryPolicy/isExpired has no unit test (code-reviewer, medium — native lane).** The Mac lane's 5e3b5c3 nil-expiry fix is safety-critical and framed as closing a fail-open, but no EnterpriseShellTests file exercises `SessionData.isExpired`/`ExpiryPolicy` — a wrong-logic edit inside an existing case would compile and pass every gate. Add a Swift unit test. Mac lane. **DONE (a9116532):** `EnterpriseShellTests/SessionExpiryTests.swift` (6 cases) pins the contract and was falsified against the old `.nonExpiring: return false` — the two blank-justification cases go red, the rest stay green. Finding along the way: `isExpired` was still a fail-open through the *decode* path — `SessionData` is `Codable` and restored from the Keychain, so a tampered blob decodes to `.nonExpiring(justification: "")`, the ignorance case relocated. Hardened so a blank-justification non-expiring session reads as expired. `SessionData.swift` wired into both the Xcode test target and the SwiftPM port. Verified: xcodebuild TEST SUCCEEDED 63/0.
