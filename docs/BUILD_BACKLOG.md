...
- [x] **A faithful end-to-end smart-charging simulator scenario (custody ground truth, MEDIUM). MODELED 2026-09-14 (DR-054, PROPOSED — owner-gated).** The journey lives in [`lib/signalgrid-simulator/src/scenarios.ts`](../lib/signalgrid-simulator/src/scenarios.ts) as five ORDERED stages plus four branches under one `custody-journey-` id prefix — the prefix is the journey identifier, the array order is the order, and both are asserted contiguously by [`scripts/src/custody-journey-proof.ts`](../scripts/src/custody-journey-proof.ts) (`proof:custody-journey`, 92/92, registered in preflight and `review-hub-ci.yml`, with a real `--self-test` mode). No type was added: `SimulatorScenario` already describes one decision and a journey is an ordered list of them, so the /v1 route, the review console and three other proofs kept working unchanged. `decisionEngine.ts` and the Swift port were NOT touched — the journey is expressed entirely in fixtures the existing rules already read. Every stage grants `allow + record_audit` and nothing else; not one branch allows, and each branch carries a RESTORED control (the same moment with only its named fault removed) that MUST grant, so a branch proves a grant withdrawn by its fault rather than a refusal in isolation. The proof also drives the eight family evaluators standing behind the stages for real (`supervision-identity`, `custody-ledger`, `device-prep`, `fallback-sequence`, `custody-beacon`, `local-authority`, `network-nac`, `change-window`) and replays the journey as an event-contract timeline through `detect.ts`. Honest limit, asserted rather than glossed: all four branch timelines resolve to the same `LEFT_PREMISES_WITHOUT_RETURN` code on different evidence — telling them apart is the still-open `CUSTODY_STALE_OR_CONTESTED` row above, and the proof pins that they are NOT distinguishable by code today. This closed the last `gap` row in the ground-truth map and moved the derived readiness headline from 94% to 100%. Original text kept for the record: (deferred design target).** The simulator carries no scenario shaped like the real workflow (badge → dock → provision → in-use → check-in) with its real failure branches (unpaired / network-down / cap-hit / dock-fault). Add one so proofs exercise the real thing rather than abstractions. Builds on the remediation-allow cascade ([`lib/signalgrid-simulator/src/remediation-allow.ts`](../lib/signalgrid-simulator/src/remediation-allow.ts)). Cloud lane. Lane: qa-engineer.
