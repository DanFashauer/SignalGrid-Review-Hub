...
- [ ] **An owner-gated hosted-model transport for YouTube URLs that `/watch` cannot download (video intake, LOW).** On 2026-09-12 `yt-dlp` got HTTP 403 from YouTube on a full-video `/watch` run and only the captions path succeeded; a 2026-09-20 owner-shared clip's recipe for the same gap is a Gemini key from Google AI Studio, which reads a YouTube URL without downloading it. That is an upload of the URL — and, for owner media, the file — to a third party, so it is the same per-machine owner decision as a Whisper key (DR-040, DR-029). If taken: the key lives outside the tree (the `~/.config/watch/.env` shape), a sibling of `transcribe-local.py` that REFUSES without the key and prints where the media went, and a `RESOURCE_INTAKE` row per use. Never a default, never on the Mac tick, never a transcript treated as a fixture (video-intake rule 3). The check that fails without it: `video-intake` on a YouTube URL that 403s produces no transcript at all. devex-tooling-engineer.
