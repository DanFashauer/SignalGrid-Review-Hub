...
- [ ] **`pnpm run loop:state` and the stop hook read a squash-landed branch as "not on the hub" when mainline ALSO moved a file the branch touched (2026-09-20, cloud lane; devex-tooling-engineer).** `scripts/loop-state.mjs` `hasLandedByContent` clears a squash-merged branch only when every file it changed is byte-identical to SOME blob in mainline's history of that path. A squash merge lands the pull request's MERGE tree, not the branch tip's tree, so a shared file that mainline changed between the branch's last mainline merge and the squash lands as a three-way merge result that never equals the branch's blob. Measured 2026-09-20 on the two branches the stop hook has named every turn since their PRs merged: `claude/build-native-ios` (#860, c7dc6610) and `claude/video-intake-beat-timeline` (#900, f7148343) each fail on exactly one file, `docs/BUILD_BACKLOG.md`, NEVER-MATCHED at history depth 111; every other file matches. Change: before calling a branch unpushed, ask GitHub for a merged pull request whose head was that branch (repo-scoped pulls endpoint, `state=closed`, `head=<owner>:<branch>`, merged_at set) and treat a merged answer as landed, reporting the merge sha; keep the byte check as the offline path and say which of the two cleared it. The check that fails without it: a fixture branch whose one changed file was also moved on mainline after the merge-base, and whose PR is merged, must be reported LANDED, not unpushed. Lane: devex-tooling-engineer.
- [ ] **A queued sim request cannot be deferred or superseded — `scripts/mac/run-requests.mjs` runs every pending request at the next tick against whatever manifest it finds (2026-09-20, cloud lane; mac-lane-steward).** The two 2026-09-18 re-mint requests will run against version 82 anyway, and the one request written AFTER the last landing (PR #910) can only follow them, never replace them; `docs/LANE_COORDINATION.md` item 7's "one request per batch, after the last manifest move" rule therefore holds only when nothing was queued earlier. Change: a `supersededBy` field on a request that names an active successor, honoured by `run-requests.mjs` (skip with the reason in the result, never a silent drop) and by `check-sim-requests.mjs` (a superseded request is reported CLOSED-BY-SUCCESSOR, never pending, and never green). The check that fails without it: a fixture request marked superseded by an existing active request must not be planned by `--plan`. Lane: mac-lane-steward.
