...
## One set of posture-allow vectors, the second guard around the engine

The same engine grants its base-trust `allow` from the PRESENCE of a
posture-bearing signal (`device.posture_observed` or `apple.ddm_declared_state`)
and reads the signal's attributes only for their known-bad members — `compliance
=== "non_compliant"`, `freshness === "stale"`, `managementState === "unmanaged"`,
`declaredState === "stale"`. So `compliance: "unknown"`, `"expired"`, a number, or
an absent key match none of those literals and the decision carries `allow`.
Measured on the real engine before the wrapper was written (eighth verdict-core
round, 2026-09-05): the clinical medication-round scenario with `compliance:
"unknown"` allows exactly as `"compliant"` does. The unknown loosened the answer.

Same discipline as remediation-allow, same reason: the engine is not edited (golden
rule 1), the rule lives around it in `lib/signalgrid-simulator/src/posture-allow.ts`
and is stated once, there and in the vector file:

> An `allow` the engine offered stands only if EVERY posture-bearing signal in the
> decision's input AFFIRMS every posture attribute the engine consults: a
> `device.posture_observed` must read `compliance: "compliant"` and `freshness:
> "fresh"`, an `apple.ddm_declared_state` must read `declaredState: "current"` and
> `compliance: "compliant"`, and `managementState`, when either carries it, must
> read `"managed"`. Any other member is UNAFFIRMED; an attribute absent where
> required, or not a string, is ILLEGIBLE. Either withholds the allow, which drops
> to the NEXT-STRICTER outcome with a named reason code; an unaffirmed or illegible
> posture raises the outcome and never lowers it; and this wrapper never moves the
> engine's own outcome in the permissive direction.

`native/shared/posture-allow-vectors.json` is the shared table — every host
outcome crossed with thirteen signal sets (affirmed on both signal types, each
unaffirmed member, each illegible shape, a second signal that must not buy
leniency, no posture signal at all). Generated by `pnpm run proof:posture-allow`
(189 checks) and never hand-edited; the proof also drives the LIVE engine — the
shipping clinical scenario still allows through the wrapper, its `compliance:
"unknown"` twin is still allowed BY THE ENGINE (the defect, reported so the
wrapper's reason to exist stays visible) and withheld BY THE WRAPPER.

`scripts/check-posture-allow-conformance.mjs` guards the table's shape and its
bindings in `preflight` and `review-hub-ci.yml`. **GATED:** the posture state every
signal set classifies to, the host outcome and reason code for every pair, no
permissive movement, non-vacuity in both directions, and byte-identity of the
committed vectors. **REPORTED, not gated:** the Swift twin (to land as PostureAllow.swift beside
RemediationAllow.swift under the EnterpriseShell services), which does not exist
yet — the Mac lane ports it against the pinned file, and the gate prints PENDING
by name until it lands, when `SWIFT_TWIN_REQUIRED` flips in the same change.
...
`proof:decision-continuity` (76 checks) guards `lib/signalgrid-core/src/continuity.ts`,
...
### Whole-repo validation does not make the reading whole-repo

Every gate above sweeps the tree. The deep independent **reads** — the verdict-core
reads, the three emit-side reads, the iOS runtime read, the Android and desktop reads,
the route cross-check — do not: each was chosen surface by surface, by someone deciding
where to look next. Nothing recorded that choice, so from outside "we reviewed the repo"
and "we reviewed the eight places we happened to go" were the same sentence.
`scripts/check-surface-review-coverage.mjs` (preflight and CI, with its self-test) closes
that. It is the SURFACE-level sibling of `check-review-coverage.mjs`, which is FILE-level;
the two answer different questions, and the file-level number can look healthy while whole
packages sit unopened.

**Coverage of the tree is asserted, not assumed.** Every tracked file must belong to
exactly one surface or to a declared out-of-scope tree, and the ratio prints on every run.
That assertion exists because this gate's own first version failed it: it enumerated
surfaces at top level only, so seven "expanded" trees swallowed their children in silence
— `artifacts/lane-messages` (115 tracked files), `artifacts/api-collection` (111),
`lab-collections` (42), the `.claude` command and hook sets, the CodeQL config,
`native/ios/mdm`, and the loose project files under `native/ios/` — roughly 330 tracked
files outside both the surface set and the "not a surface" list. It reported a surface
count over a repository it had not finished counting, which is the unearned affirmative at
one remove. The surface set is now derived to the grain of a directory: workspace
packages, then every child of an expanded tree that holds tracked files, then that tree's
loose files as their own surface, then every remaining top-level tree, then the repository
root's own files. `third_party/` and `attached_assets/` are the only declared exclusions
(vendored upstream code; owner-pasted material whose disposition is an open owner
decision), and being an EXCLUSION list means forgetting an entry makes something a surface
that demands a row rather than a file that disappears.

**Three states, not two.** A surface counts **read** only when some read covers the whole
of it. A read of a named slice is **partial** — `scope: "partial: <what>"` is required, and
without it a read of three files counted the same as a read of three hundred. Building or
running a surface is **executed**, counts towards neither, and is listed separately: the
Mac lane's `6f89457` is a real record of the iOS shell compiling and passing 8/8 on a
simulator, and all sixteen of its files are under `artifacts/sim-results/` — not one is
under `native/ios/`. Running code is not reading it. The same discipline retired this
page's own worst row: `docs/*` was seeded **read** on the strength of #375, an INDEX audit
whose commit touches three of the 203 top-level documents.

**GATED:** an uncovered tracked file, a derived surface with no row (the message prints the
line to add), a row naming a path that is gone, a declared out-of-scope tree
(`NON_SURFACE_TREES`) that matches no tracked file (the list is used only as a filter, so a
key matching zero files is invisible in the aggregate count yet reads as a hole in coverage
that is not there — fatal per-key, distinct from the aggregate "some file is excluded"
floor), a read missing its date, reviewer, record or scope, a read dated in the future (it
could never age past the staleness window), and a record that resolves to nothing. A `#NNN`
pull request is now VERIFIED against local history ("(#N)" in a commit subject): a hit
resolves, and a miss in a FULL clone is fatal because the reference is fabricated. (A miss
in a SHALLOW clone — this repo, and CI — cannot be told apart from a legitimately older PR
beyond the fetch depth, so it is REPORTED instead; see below.) A record must otherwise be a
path in this tree or a commit git can resolve. Finally, a
`docs/agent/SURFACE_REVIEW_COVERAGE.md` that has drifted from the JSON — that page is
generated by `--write` and carries no timestamp and no commit sha, which is exactly what
makes regenerate-and-compare sound for it and unsound for `docs/STATUS.md`.
**REPORTED, never fatal:** which surfaces are unread or partial, the oldest read, reads
older than 30 days, and PR references this shallow clone could not verify. Staleness stays
reported because a ledger that turns red on the calendar gets switched off, and a
switched-off gate reports nothing at all; the shallow-clone PR miss stays reported for the
same reason a full clone would gate it — the clone cannot distinguish a fabricated
reference from an old one beyond its depth, so it flags rather than swallows.

**What a green run here does not prove:** that any of those reads was good. A row says a
read happened. The record it cites is the only thing that shows what was found, and this
gate checks only that the record resolves — never what it says.
