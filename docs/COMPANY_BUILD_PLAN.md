...
12. **Commit the tiered read-list and run Tier-1 shifts 3–5** — program-manager, security-engineer, data-persistence-engineer, devex-tooling-engineer. RE-MEASURED 2026-09-26 (still open; the loop ranks open rows only): shift 3 IS read — `docs/agent/review-coverage.json` carries `2026-08-25|audited` rows for `context.ts`, `jwt.ts`, `claims.ts`, `jwks.ts`, `profile.ts`, `core.ts` and `routes/v1.ts` — but five of them have taken commits since (context `2`, jwt `2`, jwks `1`, profile `3`, core `4`; `routes/v1.ts` `10`) and are stale by the ledger's own logic; the durable path's `lib/persistence/src/decision-store.ts` and `session-store.ts` have NO row; the meta-gates `scripts/preflight.mjs`, `scripts/launch-profile.mjs` and `scripts/check-guard-registries.mjs` have NO row; `device-management-health/evaluate.ts` sits at depth `read` (2026-08-24), `local-authority/evaluate.ts` at `audited`. The other half of the row is unstarted: no `docs/agent/review-tiers.json` exists (the read-list's own line below promises one), `check-review-coverage.mjs` prints no tier line, and no ledger row carries a commit sha. Next unit, gate-shaped: commit the tier list as `docs/agent/review-tiers.json`, add a `sha` field the ledger check ratchets on, and print `Tier 1: N/25` every run so staleness is a number, then the re-reads in shift order. Check: `node scripts/check-review-coverage.mjs`. (Review-coverage lens.)
