...
18. **Make iOS port parity behavioral, not textual** — mobile-native-engineer, days. DONE 2026-09-26 (Mac lane): `scripts/src/decision-engine-parity-proof.ts` runs the TS engine over 145 deterministic cases (15 scenarios, 35 triggers × 2 contexts, 11 families pairwise) and emits `native/shared/decision-engine-vectors.json`; `native/ios/EnterpriseShellTests/DecisionEngineParityTests.swift` replays every case through `DecisionEngine.evaluate` on both build systems; `.github/workflows/ios-ci.yml` now triggers on `lib/signalgrid-simulator/**`, `lib/signalgrid-core/**` and `native/shared/**`; the parity gate's "needs a Mac" excuse is gone. The first replay went RED on 6 cases: the Swift port lacked DR-043's unauthorized-removal rule (declared as drift in `scripts/check-decision-port-parity.mjs` since PR #748) — ported the same day, declaration closed. How you'd check: `pnpm run proof:decision-engine-parity` (14 passed, 0 failed) and `cd native/ios && swift test` (111 tests, 0 failures; `xcodebuild … test` on the iPhone 17 simulator: TEST SUCCEEDED). (Native lens.)
...
48. **Native parity is textual, not behavioral** — mobile-native-engineer, days. RE-MEASURED 2026-09-26 (three of four claims closed the same day — PRs #1118 and its stacked follow-up: the port-parity gate is now backed by behavioural vectors for BOTH ports (`native/shared/decision-engine-vectors.json`, `native/shared/app-workflows-vectors.json`, replayed by `DecisionEngineParityTests.swift` and `AppWorkflowsParityTests.swift`), and `.github/workflows/ios-ci.yml` triggers on `lib/signalgrid-simulator/**` and `lib/app-workflows/**`; STILL OPEN: iOS remains carved out of the shared assist-wire conformance vectors, `native/shared/assist-wire-conformance.json`. Earlier that day: BackendService now calls only served /v1 routes (sessions start/refresh/end, context — each present in `artifacts/api-server/src/routes/v1.ts`) and refuses badge enrollment rather than faking a route (PR #387, 7554e088, 2026-09-02); the other three claims hold — the port-parity gate is vocabulary-only, iOS is carved out of the shared assist-wire conformance vectors, and `.github/workflows/ios-ci.yml` still does not trigger on lib/signalgrid-simulator or lib/app-workflows — the same gap row 18 asks to close. The
...
101. **iOS: `AppWorkflows.swift` is missing the scoped step-up release the TS planner — DONE 2026-09-26 (Mac lane, the re-port the 2026-09-12 note assigned to mobile-native-engineer in Xcode): `stepUpSatisfiedActionKeys`, the knownKeys/releasedKeys/heldKeys/allHeldReleased derivation, the per-action `actionReleased` and the "an authorized confirmer" default are in `native/ios/EnterpriseShell/Services/AppWorkflows.swift` as a literal transcription of `lib/app-workflows/src/index.ts:85-90,127-158`; `DECLARED_WORKFLOW_DRIFT` in `scripts/check-decision-port-parity.mjs` is empty. Proven behaviourally, not by shape: `scripts/src/app-workflows-parity-proof.ts` emits 368 planner decisions to `native/shared/app-workflows-vectors.json` and `native/ios/EnterpriseShellTests/AppWorkflowsParityTests.swift` replays every one (mode, summary, per-action disposition/requiresConfirmation/reason). How you'd check: `pnpm run proof:app-workflows-parity` (11 passed, 0 failed) and `cd native/ios && swift test` (113 tests, 0 failures); falsified by flipping one expected disposition in the table (the named case went red) and re-emitting. Latent on the device (the one caller gates one action at a time), so no behaviour changes today; the port stops disagreeing with /v1, which already speaks the scoped form.
...
21. scripts/preflight.mjs (841) — the per-push lane CI mirrors; a gate mis-registered here disappears quietly.
