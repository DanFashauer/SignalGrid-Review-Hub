...
8. **Read the verdict code** — principal-engineer. RE-MEASURED 2026-09-25 (still open; the loop ranks open rows only, and this one has work left): shifts 1–2 of the read-list (files 1–9 below) WERE read — `docs/agent/review-coverage.json` carries a dated `verdict-core-reader` row for all nine (2026-08-23; `store.ts` 2026-08-25), three at depth `audited` (`evidence.ts`, `store.ts`, `decisionEngine.ts`) and six at depth `read` (`engine.ts`, `decision.ts`, `policy.ts`, `resolution.ts`, `compose.ts`, `adapters.ts`) — so the row's "no named reader" premise no longer holds. What remains, and why the row stays open: the read-list's own target is depth ≥ `audited`, six of the nine sit below it, and `lib/signalgrid-core` + the simulator have taken commits since 2026-08-23 (`docs/agent/ORG_SELF_EVALUATION_2026-09-12.md` counted 16 by 2026-09-12) with no re-read. Next unit: the verdict-core-reader re-reads the six at depth `audited` against the fail-closed invariant on the current tree and dates the rows. Check: `node scripts/check-review-coverage.mjs` (prints the ledger's coverage; fails on incoherence). (Blocking; review-coverage lens.)
9. **Extend the grant-safety enumeration and mutation guard to the 14 unguarded families** — security-engineer. DONE 2026-08-20/21 (PR #219 tranche 1: network-nac, identity-risk; PR #220 tranche 2: edr-threat, device-attestation, credential-exposure, data-protection, peripheral-control, rtls-custody, macos-posture; PR #221 tranche 3: location-services, carrier, sse-egress, vuln-scan + the tier-coverage audit) and RE-MEASURED 2026-09-25: `enumerateGrantSafety` blocks in `39` proof files, `55` targets in `scripts/mutation-guard.mjs`'s `TARGETS`, the credential-exposure `scannerEnrolled` wedge caught by the tranche-2 block; #221's audit put every one of the `43` evaluator families under either an enumeration block or a hand-rolled exhaustive sweep, and #219 had already corrected this row's "14" to about 19. Checks: `pnpm run proof:grant-safety` → `summary=pass (13/13)`; `node scripts/mutation-guard.mjs` (guard coverage is the proof's own output, never a hand-written 'every'). The row read open for 36 days after it shipped. (Decision-core lens.)
...
21. scripts/preflight.mjs (820) — the per-push lane CI mirrors; a gate mis-registered here disappears quietly.
