...
One criterion is reserved to the owner. `owner-real-in-hand` has no honest machine probe and none is invented: it is met only when `attestedIn` names a decision record whose own section text carries the literal token `attests: dr-033-core-product is real in hand`. Not merely that the record exists — DR-001 through DR-055 all exist — and not merely that it names the objective, because DR-033's text already contains "DR-033". Requiring the token routes that authority through `docs/DECISION_RECORDS.md`, which the loop may never write, and a pointer at a record that does not carry the token reads `unknown` (someone typed an attestation the record does not make), never `met`.
