...
**Decision.**
1. **Explicit tier per spawn; never inherit.** Every subagent or background dispatch names its model in the brief. A spawn that omits it is under-specified and must not run — the same bar the orchestrator already applies to a worker with no falsifying check. Registered `.claude/agents/*.md` roles already satisfy this by frontmatter; ad-hoc spawns satisfy it by stating the tier in the brief.
...
4. **Never pile onto the limited model.** No spawn may share or inherit the coordinator's quota. Because each spawn is metered against its own named tier, exhausting the coordinator's quota cannot cascade into every subagent — the precise mechanism that turned one Fable limit into a lane-wide 429 storm.
...
7. **The decision-path boundary is untouched.** This record governs which *Claude* model powers a coordinating session and its subagents — a different axis from DR-029's gateway boundary and the as-yet-unmerged DR-044 tap, which keep every model, Claude or otherwise, out of the deterministic decision path. Neither is relaxed. No model reaches `lib/*`, the `/v1` decision path, a connector, or a proof.
