...
1. **Three passes, each a thin orchestrator of EXISTING agents wired to this repo's gates** — new command files under `.claude/commands/`, each carrying its own failure mode per the `COMMANDS.md` convention:
   - `/review-pass` — signalgrid-reviewer + fail-closed-auditor + code-reviewer in parallel (adding verdict-core-reader / gate-and-proof-engineer / security-reviewer when the diff warrants), findings adversarially verified, then `preflight` + `verify:breadth` green (and `test:api` N/N if the API moved). It is the local mirror of what CI and the Codex reviewer do.
   - `/refactor-pass` — refactor-cleaner + ponytail (ultra), golden rules enforced (no behavior edit to DecisionEngine.swift/AppWorkflows.swift; no Date.now()/Math.random()/I/O in a decision path; every gating switch keeps its default), behavior proven unchanged by `typecheck` + `review:invariants` + `preflight`, one runnable check left behind.
   - `/build-fix` — reproduce the failing check from output, minimal-diff fix via build-error-resolver, prove it green by re-running the SAME check; never a gate bypass; stop and report if the fix would touch a golden-rule surface.
