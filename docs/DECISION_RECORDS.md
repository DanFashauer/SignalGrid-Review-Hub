...
3. **Dark means dark, and "dark" is a structural claim, not a default value.** No route, no config file and no seed can create a live connector. The only way one comes into existence is a call to the core's `registerLiveConnector`, and **this repository contains no such call at all** — not in the api-server, not in a seed, not in a script. The seam ships as a library.
   ...
   The dark-edge resolver in `integration-bridge` is what a deployment would use to build the source. It returns `null` unless all four of: beta/prod tier, `SIGNALGRID_LIVE_INTEGRATIONS === "true"`, a non-empty `FLEETDM_API_TOKEN`, and a `SIGNALGRID_LIVE_POSTURE_TENANT` naming the tenant the connector belongs to (there is no default tenant — guessing one is how a posture read lands in the wrong one). The first three are the variables the TRANSPORT already reads; `SIGNALGRID_LIVE_POSTURE_TENANT` is the single new variable this seam introduces.

   **The destination is checked at READ time, not at arm time, and the ordering is the whole finding.** The first draft ran `validateWebhookUrl(env["FLEETDM_BASE_URL"], { live: true })` inside the resolver. `FleetDMAdapter` resolves its own base URL at fetch time from `process.env.FLEETDM_BASE_URL ?? telemetryConfig.fleetdm.baseUrl` — a different read, of a different source, possibly of a different value — so the guard passed on one address while the fetch went to another, and loopback, RFC1918 and plain-HTTP destinations the proof declared refused were reachable with the bearer token attached. A guard on a URL the transport never reads is worse than no guard, because it is quoted as one. It now lives in `fleetPostureSource`, which resolves the config ONCE through `getFleetDMConfig()`, validates that object's `baseUrl`, and hands the SAME object to `FleetDMAdapter.initialize(config)` — so the address that was checked is the address that is fetched. `proof:live-connector-sync` §9 drives the real source at a live loopback listener and asserts the listener records ZERO requests.

   Three residues are named rather than implied, because each is reachable: `validateWebhookUrl` checks the LITERAL host, so a public hostname whose DNS resolves to 169.254.169.254 passes; an IPv4-mapped IPv6 literal (`https://[::ffff:127.0.0.1]`) also passes today, and the fix for it is on a separate branch (PR #727) that is NOT in this change; and `OPERATOR_URL_FIELDS` still classifies `telemetry.baseUrl` REPORTED, not GATED — this seam gates the destination for ITSELF and changes nothing for any other Fleet read.
   ...
   **Wiring it into the api-server was written, measured and then REMOVED**, which is the most useful thing this record can report. Importing the bridge into the server makes the server depend on `@workspace/integrations`, and `scripts/check-deployment-runbook.mjs` — which derives the server's boot-read environment from its dependency closure — went from 29 boot-read variables to 170, all of them undocumented. That is a large, permanent widening of the deployed surface in exchange for a path nothing in this repository configures. So there is no process edge here, which is a stronger claim than a guarded one.
   ...
   With no configuration nothing is armed: no live connector exists, `signalSource()` still returns `"fixtures"`, and zero outbound calls are made. The proof asserts that arm first. It is **not** byte-identical to today, and an earlier draft of this paragraph said it was: `CORE_NORMALIZATION_VERSION` moves, because the covered core source changed, and that integer is stamped into every evidence snapshot and every decision. That is the stamp working as designed — the covered source DID change — but a replay against a pre-change snapshot will see a different version, and no one should be told otherwise.

4. **Fail-closed is the default outcome, not the error path.** An unreachable, timed-out, unauthenticated or malformed live read writes ZERO signals, reports the run `partial`, and marks the connector `degraded`. It never synthesizes a healthy reading, never reports `success`, and never coerces an unknown management state onto `managed: false` — a host whose management value nothing recognises is DROPPED. A response that yields NO usable records lands in the fail-closed arm, and that now includes an explicit `{"hosts": []}`: a 200 with an empty inventory is what a revoked token scoped to nothing, a wrong team filter and an emptied Fleet all return, and the first draft read all three as a clean sync of an empty fleet — `success`, connector `healthy`. An empty result set is an UNKNOWN. A deployment genuinely running an empty Fleet reads `partial`/`degraded`, which is the correct direction to be wrong in.

   The same rule widened the record shape: `identityRef`, `identityEnabled`, `encrypted` and `osSupported` are now OPTIONAL on `FixturePostureRecord`, because a device-posture source answers nothing about a person and a boolean cannot hold "unanswered" — absent emits no signal, exactly as `baseline`, `managementHealth` and `localAuthority` already did.

   **WHAT IS NOT CLOSED, and it is a real gap, not a caveat.** An earlier draft of this term said "absent fresh signals age into `unverified`, which raises the assurance bar." They do not. Posture freshness is graded ONCE, at sync time, against `record.lastSyncAt`, and stored as the `posture_freshness` signal's VALUE; `buildEvidence`/`readFreshness` read that value back verbatim at evaluation time. A reading that graded `fresh` yesterday still reads `fresh` today however long the connector has been degraded — so a failed live read leaves a stale healthy posture standing rather than letting it lapse. This is pre-existing behaviour of the whole core (the dock and shift paths stamp freshness the same way), not something this seam introduced, but this seam is the first place that claimed otherwise and the claim is withdrawn. **Deriving freshness at read time, against the injected evaluation clock, is the fix, and it is an owner decision, not a comment**: it changes the outcome of every decision the core makes against every existing fixture and every persisted snapshot, and it needs its own record. What a failed read IS guaranteed to do: write nothing, report `partial`, mark the connector `degraded` — and in a store whose only posture source is that connector, leave the category absent, which grades `missing` and does raise the bar. `proof:live-connector-sync` §5 drives exactly that store.

5. **Read-only, and the word is doing work — but the work is done by what is CALLED, not by a guard.** The seam's only outbound call is `FleetDMAdapter.getHosts()`, a GET. The adapter's one non-GET — `runQuery()`, which POSTs arbitrary osquery SQL to real hosts — is not called here and additionally requires `SIGNALGRID_ALLOW_LIVE_QUERY=true`. No writes, no device actuation, no remediation; actuators were deleted from `nac/` and `uem/` rather than gated, and nothing here reopens that door.

   A first draft wrapped the read in `createReadOnlyGuard` and called it `guardReadOnly("GET")` — with a string literal, so it could never throw — and the proof exercised that guard directly rather than the source. **The guard has been deleted rather than dressed up.** A guard invoked with a constant proves nothing and reads in review as if it does, and "ENFORCED" was the wrong word for it. What §9 of the proof asserts instead is the strongest property that is actually reachable offline: on a refused destination, NO request of ANY method leaves the process.
...
**Reversal is `git revert`, and an earlier draft of this section was wrong about it.** It claimed reversal was one line — narrow `ConnectorMode` back to `"fixture"` in `lib/signalgrid-core/src/types.ts` and let the compiler do the rest. It is not: `engine.ts`'s `registerLiveConnector` hard-codes `mode: "live"` in the row it writes, so narrowing the union alone fails `pnpm run typecheck` rather than producing a working disarmed tree. The type does not enforce the reversal; it only reports that one is half-done.

The reversal is therefore: revert this PR. It deletes three files — the core's live runner, the bridge's dark-edge resolver and the new proof — folds the shared normalizer back inline into the fixture entry point, drops the `await` at the `/v1/connectors/:id/sync` route, restores `ConnectorMode` to one member, restores the claim sites to their current wording (at which point that wording is true again), and re-runs `node scripts/generate-core-normalization-version.mjs` so the stamp returns to its pre-change value. No data migration, no persisted state, no external system was ever contacted, so there is nothing to undo outside the tree.

A reversal to the disarmed-but-present state is also valid and is the expected first response to any concern, but it is two edits, not one: narrow `ConnectorMode`, and remove or re-type `registerLiveConnector` so it no longer writes a `"live"` row. The compiler will name the second one the moment the first is made.
