...
**Decision.**
1. **The coordinator writes specs and reviews; it dispatches everything else.** Patch scripts, PR bodies, commit messages, log and job-list parsing, gate runs, measurement reads and doc regeneration go to Haiku (bulk mechanical), Sonnet (reads, verification, building) or Opus (judgment), per DR-047 rule 2. The stage table is in `.claude/skills/orchestrator-over-workers/SKILL.md` ("Which model runs a stage — the coordinator runs none of the bulk"). The coordinator doing a bulk stage itself is a defect, recorded as a lesson.
...
**What no gate can observe.** Which tier ran a stage inside a session is invisible to every check in this tree. The record of it is the `TIERS THIS SESSION` line in the `docs/agent/LOOP.md` STATE block, written honestly each session, including the stages the coordinator ran itself.

**Evidence.** The six lessons L1–L6 in `docs/agent/LESSONS.md`, each with its quoted output or path. L1's landing ships with this record: `scripts/preflight.mjs` now runs `scripts/mac/free-test-port.sh` first on every platform (a planted process whose argv carries this tree's api-server path is reaped; a sibling checkout's and an unrelated process survive). `node scripts/check-lessons.mjs --self-test` plants each failing row shape and asserts it fails.

**Consequences.** The coordinator's own tier is spent on specs and review. Every cycle leaves its mistakes where the next one reads them, and a lesson without a landing is visible in the gate's report. The first two open slices are backlog rows: the scripted measurement tranche (L2's landing) and a per-lane, per-skill MCP roster with a derived tool count (rule 3's first slice).

**What does not change.** DR-047's tiers and fallback rules, DR-054's raise-your-hand law, DR-056's objective loop, the verdict enum, the determinism invariant and the Decision Envelope.
