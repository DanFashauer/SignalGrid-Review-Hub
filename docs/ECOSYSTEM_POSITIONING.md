...
| HIS / EHR (clinical system of record) | Epic, Oracle Health, MEDITECH, athenahealth, Veradigm, Altera, GE HealthCare | The clinical system of record — patient chart, EHR/EMR, CPOE, orders, results, clinical documentation, scheduling — and the DOMAIN safety of the care workflow (patient lookup, clinical guidelines); interoperability via HL7/FHIR/DICOM. | Nothing from the record. SignalGrid never reads patient or clinical data; where an approved integration exists it may take session or workflow context only, never chart content. | A runtime allow/step-up/restrict/deny before the worker reaches the host clinical app, plus audit evidence. It writes nothing into the record. | SignalGrid is not an HIS/EHR and does not replace clinical records, CPOE, orders, or documentation. Golden rule 3: patient lookup, clinical guidelines and other domain safety live in the host clinical apps, never in SignalGrid. |
