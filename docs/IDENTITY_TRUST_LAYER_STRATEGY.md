...
> **SUPERSEDED 2026-09-06 in its proof order — do not follow the "Entra ID + Intune first" sequence.**
> Last substantively written 2026-08-03. DR-012 (2026-08-22) redirected the proof stack:
> Fleet-first, Microsoft when a prospect brings a tenant. The identity-signal model here
> (IAM as a source, never a replacement) still stands and is what
> `docs/ECOSYSTEM_POSITIONING.md` routes to; the two ordering sentences are annotated in
> place.
...
The first practical proof should remain **Entra ID + Intune** *(as written 2026-08-03; superseded by DR-012 — Fleet-first, Microsoft when a prospect brings a tenant)* because it combines identity, Conditional Access context, and device posture in a single Microsoft path that can be validated with deterministic Graph-backed or fixture-backed evidence before broader connector work begins.
...
1. **Entra ID + Intune first proof** *(superseded by DR-012; Fleet is the first proof, and `proof:live-fleet` records it)*: validate identity plus device posture using Microsoft Graph / Graph SDK reads or deterministic public fixtures.
