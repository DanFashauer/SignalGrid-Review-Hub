...
**Aligned to Apple's canonical schema.** Each normalized posture field carries its provenance in [`apple/device-management`](https://github.com/apple/device-management) (Apple's MIT-licensed, machine-readable MDM + Declarative Device Management schema, pinned at version 27.0): `sip` → `SystemIntegrityProtectionEnabled`, `fileVault` → `FDE_Enabled` (+ DDM `diskmanagement.filevault.enabled`), `firewall` → `FirewallSettings.FirewallEnabled`, and so on — plus the Managed Device Attestation leaf-cert OIDs (e.g. the attested SIP status `1.2.840.113635.100.8.13.1`) reserved for a future hardware-rooted attestation tier. On-device-only signals with no MDM/DDM key (Gatekeeper, XProtect, system extensions) are declared as such rather than given a fabricated key. `pnpm run proof:macos-apple-schema` (53 checks, offline) asserts every posture field is mapped, every referenced key is in the pinned Apple catalog, and — since two subsystems pin this same upstream — that `macos-posture` and `ddm-connector` name the SAME release, an invariant `ddm-connector` stated in a comment but nothing enforced. So reconciling only one of the two alignments fails. The proof compares against the catalog pinned in this repository, not Apple's YAML, so a new OS release is re-verified against upstream by hand when the pin moves — 27.0 was, item by item, on 2026-09-18 (all 28 references unchanged from 26.4). Aligning to these names is adoption of a public standard, not a code dependency or vendor partnership.
...
A reported failure also carries a **root-cause evidence** axis. The management portal
reports the RESULT; the reason lives on the device (agent logs, event logs, the MDM
diagnostic report, certificate request state, script exit codes). Without the axis, "the
portal says enrollment failed and nobody looked" and "the log was collected and a
certificate rejection is the confirmed cause" emitted the same reason code. The outcome
was `restrict` in both cases and still is — this changes no verdict; it stops the
connector asserting a diagnosis it cannot support, and stops it misrouting, since which
owner picks it up (PKI, app packaging, network, identity) is exactly what the absent
evidence would have said. Only an explicit `available` earns `ENROLLMENT_FAILED`;
`unavailable`, `unknown` and `not_supported` all yield
`ENROLLMENT_ROOT_CAUSE_UNVERIFIED`. `not_supported` is deliberately not promoted: a
source that structurally cannot explain itself still has not explained itself. The axis
is vendor-neutral by design — vendor-specific artifacts belong in `sourceReferences`, not
in a normalized contract that Jamf, Workspace ONE and Fleet also map into. This is the
mirror of the absent-collection law: that one says *nothing observed is not the same as
nothing wrong*; this one says **a status is not a diagnosis**.

Proven fully offline by `pnpm run proof:device-management-health` (207 checks, no network), 0 mismatches on all three enumerations via the shared grant-safety harness: the **normalized space (21,600 states)** against the evaluator alone, the **raw wire space (1,354,752 reports)** carrying the values a real bridge emits — junk enum spellings, JSON nulls, string-quoted booleans, numbers, arrays, objects, absent keys, and a snake_case aliased extra key — and a **parse-fidelity pass** over that same space asserting `reportIntegrity` against an independent allowlist. Exactly three raw reports grant — one per consistent channel shape — so the seven-way confirmation is provably tight. The third pass is what makes the integrity conditions provable at all: grant-ness alone cannot see them, since every malformed value already normalizes to a denying sentinel. The whole set is mutation-tested rather than assumed: **11** deliberate weakenings of the normalizer and evaluator were each applied and reverted, and every one is now caught. Two of them — deleting either of two terms in the consistency guard — previously left the proof green, which is precisely the load-bearing-but-unproven state the third pass exists to prevent, so each term now has a fixture of its own. Live calls are gated exactly like every other connector: fixture mode unless tier is beta/prod **and** `SIGNALGRID_LIVE_INTEGRATIONS=true` **and** `DEVICE_MANAGEMENT_HEALTH_ACCESS_TOKEN` is set. SignalGrid reads and decides on the evaluated management state; every signal is read-only, and this is not a vendor partnership or certification claim.

**Transports.** Two, selected explicitly by `DEVICE_MANAGEMENT_HEALTH_TRANSPORT`. `bridge` (the default, unchanged) targets a bespoke management-plane endpoint. `graph` targets the real `GET https://graph.microsoft.com/v1.0/deviceManagement/managedDevices/{id}`, read-only, with a `$select` naming exactly the seven properties the mapping reads so the tenant returns nothing more. The default was deliberately NOT moved to `graph`: that would silently repoint an existing beta/prod deployment at a different upstream on upgrade. **One Graph call answers five of the seven axes.** `policyDrift` needs per-device configuration/compliance policy states plus a baseline Graph does not hold, and `remediationHealth` needs device health-script run states — a different resource. Both are returned as `unknown`, which is the correct answer and the fail-closed one: an unknown raises assurance rather than granting on a fabricated measurement. Freshness compares against a clock that must be passed in; the transport refuses to build without one.
...
- a link that is **associated** while the bridge affirmatively reports nothing got through, or that is failing at **DHCP** or **DNS**, or that is confirmed **local-only** (local traffic carrying while WAN egress affirmatively fails — the warehouse case where "the Wi-Fi and local servers are fine, only the control plane is dark") → `alert`, each with a critical finding. These are confirmed facts about the link, not gaps in what we know. The `local_only` rung is **annotate-only by owner direction**: it names the state at the same severity as the other confirmed rungs; whether it may ever *soften* an offline restriction for named workflows is an owner decision deliberately not wired (completion plan §12.1);
...
Proven fully offline by `pnpm run proof:link-usability` (167 checks, no network), 0 mismatches on all three enumerations via the shared grant-safety harness: the **normalized space (7,560 states)**, the **raw wire space (241,920 reports)** carrying the values a real bridge emits, and a **parse-fidelity pass** over that same space asserting `reportIntegrity` against an independent allowlist. Exactly **three** shapes grant — one per coherent roam pair — and that count is pinned **over the enumerated wire space**, so a fourth route within it is a test failure rather than a silent widening (adding the `local_only` rung grew both spaces and left the granting count at three, which is itself the annotate-only claim holding under enumeration). That qualifier is load-bearing and was missing: the proof itself demonstrates a route outside the enumeration, since a Proxy that lies in `ownKeys` while still answering `getOwnPropertyDescriptor` keeps its values readable and grants. The proof also audits the whole raw space for the cite-a-disbelieved-claim pattern on both contradiction relations: across **24,192** ladder-contradictory and **21,600** roam-contradictory reports, not one cites the disbelieved half. `linkUsable` is separately asserted to agree with `action === "none"` on every one of the 7,560 normalized states — a mutation widening it survived until that check existed. Live calls are gated exactly like every other connector: fixture mode unless tier is beta/prod **and** `SIGNALGRID_LIVE_INTEGRATIONS=true` **and** `LINK_USABILITY_ACCESS_TOKEN` is set. SignalGrid reads and decides on the evaluated link state; every signal is read-only, and this is not a vendor partnership or certification claim.
