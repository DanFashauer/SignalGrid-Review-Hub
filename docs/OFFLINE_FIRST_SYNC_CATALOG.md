...
| **Device posture** | the connector families under `lib/integrations/src/integrations/` → `lib/posture-composition` | **COVERED.** Where freshness is graded it is caller-posed — `fixAgeSeconds` (`rtls-custody`, a deferred family) and `maxObservationAgeSeconds` (`facility-trust-graph`) — never `Date.now()`. Not every family grades it, and the honest form of that is to say so: `uem` carried a check-in age no evaluator read, and it was removed 2026-09-01 rather than left standing as coverage. |
