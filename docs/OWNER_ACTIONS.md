...
3. **Company vs product separation — ratify or redirect** (2 min read):
   `docs/COMPANY_VS_PRODUCT.md` answers your question with a recommendation —
   one repository with an explicit boundary, because the truth gates only bite
   while company claims and product code share a tree — plus the named
   triggers that would flip it to a split. Say "agreed" or say what to change.
...
7. **Classify four candidate connector families** the freeze is holding —
