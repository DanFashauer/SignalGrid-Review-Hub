...
| `connector-emulator-smoke-evidence` | artifacts/connector-emulator/evidence.json (written during the run; not in the repository) | Small workflow evidence manifest for post-merge review. |

`results.json` is committed and regenerated by `pnpm run proof:connector-emulator`.
The evidence manifest is not: the `Generate smoke evidence manifest` step of
`.github/workflows/connector-emulator-smoke.yml` writes it inside the runner from
that run's GitHub environment, so it exists only as the uploaded artifact — there
is nothing at that path to open in a checkout.
