...
(`pnpm run proof:signalgrid-core`, 61 invariant assertions) must keep passing
...
- **Deterministic core proof, unchanged.** `pnpm run proof:signalgrid-core`
  (61 invariant assertions: correct outcomes, fail-closed, tenant isolation,
  RBAC, auth-fails-closed, tamper-evidence, determinism) runs against the
