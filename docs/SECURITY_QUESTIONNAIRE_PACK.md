...
| Does the product write to source systems? | No write to any source system exists on the launch surface; adapters supply evidence only. The one sync trigger route dispatches by connector mode; a live-mode connector exists only where a process calls core.registerLiveConnector, and this repository contains no such call at all, so every connector it holds is fixture-mode. The live path, where a deployment arms one, is a GET-only read in any case. | `scripts/launch-profile.mjs` (the `/v1/connectors/{id}/sync` entry) | Built and gated |
