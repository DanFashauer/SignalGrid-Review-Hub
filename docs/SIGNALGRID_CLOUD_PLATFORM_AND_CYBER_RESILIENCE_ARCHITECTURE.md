...
## The ten cloud-architecture concepts, mapped to this platform

**Source, named rather than absorbed silently.** The owner shared an infographic,
*"10 cloud architecture concepts"* by **Rajender Ponnala** — an Azure-flavoured
one-pager covering cloud architecture, load balancing, scalability, caching,
sharding, replication, message queues, microservices, fault tolerance and
observability, closing with *"start with the business problem, understand workload
characteristics, consider non-functional requirements, choose the pattern and
services, evaluate trade-offs."* The image is third-party and is **not committed**;
it is described here and logged in `docs/agent/RESOURCE_INTAKE.md`.

The reason to map it rather than file it is the footer. Every row below starts
from the business problem this platform has — *should this actor perform this
action on this resource in this workflow right now, and what happens next* — and
several of the answers are **deliberate refusals**. A generic cloud pattern applied
to a fail-closed decision path can invert it: a cache makes a stale `allow`
survivable, a retry makes a decision slow instead of honest, a queue makes a
verdict arrive after the person has walked away. Where a row says *does not*, the
absence is the design.

Two standing constraints bind the whole table. Nothing here is a **deployment-target
claim**: this repo is a Node/esbuild control plane plus a `/v1` decision API,
containerised (`Dockerfile.api`, `docker-compose.prod.yml`, `nginx.conf`), and it is
not bound to Azure, AWS or any other cloud — layer 2 of the status table above is
*NOT BUILT* and stays that way. And nothing here is a shipped-capability
claim: the launch profile and the publication boundary still govern what may be said.

| # | Concept | What it means here | What the tree has | What it deliberately does not do, and why | Backlog |
| --- | --- | --- | --- | --- | --- |
| 1 | **Cloud architecture** | The ten-layer table at the top of this page is the architecture; this repo implements the bottom half and *consumes* the top. | `artifacts/api-server` (Node/esbuild; `/v1` decision API + `/cp/v1` control plane), `lib/*` as the domain packages, `docs/DEPLOYMENT.md` for the self-hosted shape. | No cloud foundation layer, no account/OU/region model, no cloud provisioning — layer 2 is NOT BUILT and assumed. An architecture page that drew it solid would be implying capability, which is the failure this page opens by refusing. | None. Out of scope by design, not by omission. |
| 2 | **Load balancing** | Many host apps calling one decision surface; the surface must answer identically whichever instance answers. | `nginx.conf` fronts `api` and `web` upstreams; `Dockerfile.api` and `docker-compose.prod.yml` build the runtime image. The decision itself is pure — same normalized inputs and policy version, same verdict — so instances cannot disagree about a decision. | It does not claim horizontal scale-out works today. Two per-process memory stores sit in the request path and say so in their own headers: the idempotency replay cache (`artifacts/api-server/src/middlewares/idempotency.ts`) and the fixed-window rate limiter (`artifacts/api-server/src/middlewares/rateLimit.ts`). Behind a balancer, replay coverage and throttling become per-instance. | Yes — a shared store for both before multi-instance is claimed. |
| 3 | **Scalability (up / out)** | How many decisions per second, and where the knee is. | `pnpm run test:load` and `pnpm run test:stress` drive the `/v1` HTTP surface under concurrency; `pnpm run bench:decision-latency` and `pnpm run bench:decision-throughput` measure the in-process core with no HTTP, no connector and no database. The gap between the two numbers is the transport. | **Correctness under concurrency is GATED; throughput, percentiles and the saturation knee are REPORTED and not gated** — a latency threshold on a shared runner is a flaky gate, and a flaky gate gets switched off, which is worse than no gate. Nothing here claims a capacity figure. | No. The reporting posture is the decision. |
| 4 | **Caching** (TTL, invalidation, eviction, consistency, stampede) | The one place a cache would obviously pay — reuse the last verdict — is the one place it must not exist. | The only TTL'd store on the wire is the idempotency replay window in `artifacts/api-server/src/middlewares/idempotency.ts`: principal-scoped, 2xx only, FIFO-evicted at a bounded key count, **five minutes**. | **There is no decision cache and no evidence cache.** The replay window exists for transport retries — a response lost in flight, a crash-recovering host app re-sending — and it was cut from 24 hours to 5 minutes precisely because a cached decision-bearing 2xx is *posture pinning*: an `allow` replayed a working day after the posture it graded had degraded to `deny`. Freshness is **graded, not cached** (`postureFreshness`, the staleness axes on every dimension); a stale signal raises assurance rather than being served again. | No. A cache in the decision path is a doctrine line, not a gap. |
| 5 | **Sharding / partitioning** (partition key) | The partition key is `tenantId`, and the property that matters is not throughput but isolation. | Every tenant-scoped reader goes through `scoped()` in `lib/signalgrid-core/src/store.ts`; `pnpm run proof:isolation-scope` runs **every** such reader against **every ordered pair** of seeded tenants, so a new reader added without a scope is a failure rather than an omission nobody notices. Durable rows are keyed on tenant in `lib/persistence`. | No horizontal shard map and no per-tenant database. PostgreSQL row-level security is a **private-core design target**, not something built here (`docs/PRIVATE_CORE_HANDOFF.md`, `docs/SECURITY_CONTROLS_MATRIX.md`); the served core's isolation is application-layer and proven as such. | Yes — RLS as a durable-store gate when the durable core is built. |
| 6 | **Replication (≠ backup)** | Backup answers "can we get the data back"; replication answers "does the service survive losing a node". This repo answers only the first, and says so. | `docs/BACKUP_AND_RESTORE.md`: a logical dump, a verified restore, a CI-exercised migration path, and an exportable tamper-evident audit chain. | It states its own limits rather than implying more: **no point-in-time recovery, no WAL archiving, no RPO/RTO claim.** Replication is not implemented and is not claimed anywhere — and the doc records why a replica would not be a substitute for the chain's integrity property: a replica of a chain truncated from the end replicates the truncation (*"What the chain cannot see: deletion from the end"*). | No. A replica/PITR posture belongs to the operator's deployment; claiming one here would be a deployment claim this repo may not make. |
| 7 | **Message queues** (dead-letter, retry, duplicate detection) | Queues belong on the way **out** of a decision, never on the way in. | Outbound: `lib/integrations/src/integrations/webhooks/` — `dispatch.ts` with retries, a DLQ and delivery logs, `retry.ts` with exponential backoff plus jitter, `store.ts` Redis- or memory-backed. The core models the same shape deterministically: `lib/signalgrid-core/src/webhooks.ts` records a `delivered` / `failed` / `dead_letter` outcome with a backoff schedule that is **recorded, never awaited**, so retry behaviour is demonstrable without a clock. Duplicate detection on the inbound side is the `Idempotency-Key` header. | **Signal ingestion is not queued, deliberately** — `/v1/decisions/evaluate` is synchronous and in-process, because a queued verdict is a verdict that arrives after the person has walked away from the device. And there is **no broker**: no queue library (Service Bus, Kafka, RabbitMQ, BullMQ, SQS) is a dependency of any workspace package. | Yes — a durable outbound queue for the cascade emitters, fail-closed: an unreachable backend leaves the item queued and reports it, and never reports a delivery it did not make. |
| 8 | **Microservices** (API management, per-service DB) | The service boundary here is the **contract**, not the process count. | One Node process serves `/v1` and `/cp/v1`; the domain is split into workspace packages (`lib/signalgrid-core`, `lib/integrations`, `lib/orchestration`, `lib/incident-playbook`, `lib/persistence`, …) with dependency rules instead of network hops. API management is real and gated: `lib/api-spec/v1-openapi.yaml` is the published contract, with generated clients (`lib/api-zod`, `lib/api-client-react`), a Bruno collection and `pnpm run check:postman`. | No mesh, no per-service database, no split. The determinism invariant — same normalized inputs and policy version, same verdict and same rationale — is provable in one process and becomes an integration test across five. The served core holds **no database at all**; `lib/persistence` is the durable decision/session store behind the readiness probe. | No. |
| 9 | **Fault tolerance** (retry, backoff, circuit breaker, timeouts, failover) | **Fail-closed *is* the fault-tolerance model.** An unreachable dependency does not degrade the answer; it raises assurance. | Timeouts: `AbortSignal.timeout` on the read transports, e.g. `lib/integrations/src/integrations/device-management-health/graph-transport.ts`. Hostile-wire behaviour is proven over a real socket — `pnpm run proof:graph-wire` drives Graph-authentic throttling, 5xx, auth failure, malformed bodies and paging, and pins the page-cap refusal (`incomplete_read`) that stops a truncated inventory reading as a complete one. Partition behaviour: `lib/signalgrid-core/src/continuity.ts` — an offline authority can never relax a stricter connected decision, and last-write-wins is rejected by name because a settable clock would make changing the date a grant primitive. Retry with backoff exists on the **outbound** emitter path only. | **No retry and no circuit breaker inside the decision path.** A retry there converts an honest `unknown` into a slower answer while a person waits at a door; the fail-closed value is available immediately and is the safe one. A circuit breaker's job — stop hammering a sick dependency — is already done by "unreachable ⇒ unknown ⇒ raise assurance", which costs one attempt. The one circuit breaker named anywhere in this tree belongs to the agent/build AI gateway (`docs/AGENT_GATEWAY.md`), which DR-029 bars from the decision path entirely. Multi-region, availability zones and health-check failover are layer-2 concerns and are neither built nor claimed. | Yes — report-only health/breaker state for the outbound emitters, so a dead ITSM backend is visible without ever becoming a reason to relax a verdict. |
| 10 | **Observability** (logs, metrics, traces) | Two questions, not one: what is this platform doing, and *what is another system's silence worth as evidence*. The second is the one that earns a decision dimension. | Metrics: `GET /metrics` renders Prometheus text (`artifacts/api-server/src/app.ts`), global aggregate only — no tenant label, no payloads — optionally bearer-protected by `METRICS_TOKEN`, with route labels taken from the Express route that **matched** so an unauthenticated caller can mint at most one label series. A live lane carries it end to end: `scripts/lab/otel-collector.yaml` and `scripts/run-live-lanes.sh --only telemetry` assert `signalgrid_up` traversing api `/metrics` → OTel Collector → Prometheus. Decision-plane SLOs and error budgets: `lib/reliability`. Silence-as-evidence: the `observability-integrity` dimension grades collection state, stream fidelity and staleness, and its `silenceIsEvidence` field is true only at full fidelity inside the stream's declared interval. | **No OpenTelemetry SDK is a dependency of any workspace package, and nothing in the tree emits a trace** — the only `@opentelemetry` string in the source is an externals entry in the api-server bundler config (`artifacts/api-server/build.mjs`); OpenTelemetry appears here as the collector in the live lane and as the reference *shape* the integrity dimension grades against. No collector, no agent, no storage, no query engine, no SLO plane for observed workloads: collecting is the platform's job. | Yes — trace emission from the `/v1` surface, if and when a deployment asks for it; it is not needed to decide. |

### Where this tree already speaks Microsoft, and what that does not mean

The founder's operating world is Microsoft — Entra ID, Intune, Defender,
ServiceNow — and the tree reflects it, read-only and fixture-gated in every case:
`lib/integrations/src/integrations/graph` (Microsoft Graph posture, with a mock
transport and a real-socket proof), `device-management-health/graph-transport.ts`
(the Intune `deviceManagement/managedDevices` read), `identity-risk`,
`telemetry/mde.ts`, and eight ITSM vendor emitters led by ServiceNow.

**That is signal-source coverage, not a platform choice.** Speaking Graph is how
the grid reads a Microsoft estate; it says nothing about where SignalGrid runs, and
this page adds no deployment-target claim in either direction. The same rule that
makes the source systems replaceable (`docs/PURPOSE.md`, *The system underneath is
replaceable*, DR-035) makes the hosting replaceable: a grid that required one
cloud would be the vendor lock it exists to remove.
