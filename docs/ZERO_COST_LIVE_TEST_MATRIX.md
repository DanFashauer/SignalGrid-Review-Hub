...
| --- | --- | --- | --- |
| Every decision dimension | 151 deterministic proof scripts (`pnpm run proof:*` at the repo root, measured 2026-09-18; 150 are `scripts/src/*-proof.ts` and one, `proof:decision-palette`, is a root-only alias), grant-safety enumeration (millions of raw states), mutation guard (measured 2026-08-26: 1,367 mutations, 1,287 killed, 80 documented-inert, 0 survivors — swept daily by the scheduled lane across four shards, counts reprinted by every run), figure/registry/count drift guards | The decision logic itself, adversarially | $0, every commit |
...
| API surface | API integration test (boots the real server; a route-coverage check proves all 38 registered /v1 routes are exercised, and the test itself verifies that this very number still matches the server), OpenAPI contract check, Postman collection kept in sync | The wire contract | $0 |
