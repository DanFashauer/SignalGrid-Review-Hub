...
| --- | --- | --- | --- |
| Every decision dimension | 153 deterministic proof scripts (`pnpm run proof:*` at the repo root, measured 2026-09-23; 152 are `scripts/src/*-proof.ts` and one, `proof:decision-palette`, is a root-only alias), grant-safety enumeration (millions of raw states), mutation guard (measured 2026-08-26: 1,367 mutations, 1,287 killed, 80 documented-inert, 0 survivors — swept daily by the scheduled lane across four shards, counts reprinted by every run), figure/registry/count drift guards | The decision logic itself, adversarially | $0, every commit |
