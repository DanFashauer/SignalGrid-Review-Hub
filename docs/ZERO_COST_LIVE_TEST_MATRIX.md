...
| API surface | API integration test (boots the real server; a route-coverage check proves all 37 registered /v1 routes are exercised, and the test itself verifies that this very number still matches the server), OpenAPI contract check, Postman collection kept in sync | The wire contract | $0 |
