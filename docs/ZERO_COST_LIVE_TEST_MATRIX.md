...
| --- | --- | --- | --- |
| Every decision dimension | 144 deterministic proof scripts (`pnpm run proof:*` at the repo root, measured 2026-09-05; 143 are `scripts/src/*-proof.ts` and one, `proof:decision-palette`, is a root-only alias), grant-safety enumeration (millions of raw states), mutation guard (measured 2026-08-26: 1,367 mutations, 1,287 killed, 80 documented-inert, 0 survivors — swept daily by the scheduled lane across four shards, counts reprinted by every run), figure/registry/count drift guards | The decision logic itself, adversarially | $0, every commit |
...
| API surface | API integration test (boots the real server; a route-coverage check proves all 35 registered /v1 routes are exercised, and the test itself verifies that this very number still matches the server), OpenAPI contract check, Postman collection kept in sync | The wire contract | $0 |
...
| Browser surfaces | NEW: Playwright E2E (`pnpm run test:e2e`) — 53 tests in 10 spec files as listed on 2026-09-06 by `cd scripts && npx playwright test --config playwright.config.ts --tsconfig ../tsconfig.base.json --list` (it prints `Total: 53 tests in 10 files`; this cell said 35 before that and 15 before that — quote the runner, not this cell; the "~126 content-bearing assertions" once counted alongside the 35 has no tool that reports it and is not re-derived here) across the review console, public website, admin console, desktop client, and mobile PWA (admin, desktop, and PWA wired to a live api-server); ~38s wall clock including all six app builds. The suite shipped with one deliberately-red test that PROVED a real product gap: the review console never surfaced the Battery health row even though the core correctly returned `restrict`/`BATTERY_FAILING` — the console's scenario list had no failing-battery entry. The one-line scenario fix landed in that same change, taking the suite to 15/15 **as it stood then** (it has since grown to the count listed above). That red-first test is the pattern to keep: E2E asserts what a human sees, not what the core knows. | What a human actually sees renders and says the right thing | $0 |
