...
- **Mac liveness — RESOLVED.** `mac-lane-tick` now runs under launchd every 5 minutes on the owner's Mac (`scripts/mac/lane-tick.sh`, installed via `scripts/mac/install-launchd.sh`), heartbeating to `artifacts/agent-heartbeats/mac-lane-tick.json` every 5 minutes — see that file for the latest fire. See `docs/LANE_COORDINATION.md` ("`scripts/mac/lane-tick.sh` runs every 5 minutes from launchd"). The residual risk is staleness, not absence: if the tick stops heartbeating, Swift/evidence winners go UNVERIFIED and cannot ship from cloud alone. *Mitigation:* mark them never-green and escalate staleness; cloud never blocks.
...
2. **Mac tick.** ~~Will you run `bash scripts/mac/install-launchd.sh` on your Mac? Without it, Swift/iOS/evidence audits never fire unattended.~~ **RESOLVED:** the launchd tick is installed and firing — `mac-lane-tick` runs every 5 minutes and heartbeats to `artifacts/agent-heartbeats/mac-lane-tick.json` — see that file for the latest fire — as the "Mac liveness — RESOLVED" risk bullet above records. The residual concern is staleness, not absence, and is handled there (mark UNVERIFIED and escalate; cloud never blocks).
