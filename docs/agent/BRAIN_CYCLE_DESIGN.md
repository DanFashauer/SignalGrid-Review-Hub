...
- Activation is owner-authorized (2026-09-13, "Activate it now"); what remains is a **registry-first bootstrap in this exact order** — NOT `create_trigger` first (a same-session activation on 2026-09-13 was reverted for that ordering error, Codex #705): (1) land the `brain-cycle` row on `SignalGrid_Alpha` and prove a Slice-2 dry run end-to-end (its proof lands in the per-cycle board `artifacts/brain-cycle/<sha>/`, NEVER at `heartbeatPath`) — KEEP the row `awaiting-activation` through this and the next step; its liveness is measured from `awaitingSince` (REPORTED, not fatal, up to the 30-day bound), while `authorizedOn` stays the 2026-09-09 owner-consent instant and is NEVER overwritten (the 2026-09-13 activation go lives in `authorizationEvidence`, not by rewriting the consent date); (2) THEN `create_trigger` (self-session, cloud) and transcribe the `trig_*` id into the row; (3) let the first REAL scheduled fire write the heartbeat at artifacts/agent-heartbeats/brain-cycle.json (un-backticked: it does not exist until the first fire) — do NOT hand-write a heartbeat before the trigger exists (`check-scheduled-routines.mjs` reads any heartbeat as proof of a fire). ONLY in the change that records that first heartbeat, flip the row to `active` (drop `awaiting-activation` — the gate FATALs on `awaiting-activation` + a fired heartbeat). Because the flip to `active` follows the first heartbeat, the gate's active-no-heartbeat window (measured from `authorizedOn`) never applies — so the consent ledger need not be, and must not be, falsified to keep the gate green. If the bootstrap ever slips past the 30-day `awaitingSince` bound, re-scope the wait by refreshing `awaitingSince` (a liveness field), never `authorizedOn`.
