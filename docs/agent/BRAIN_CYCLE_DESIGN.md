...
### Slice 3 — the account trigger + owner GREEN switch (owner-authorized 2026-09-13; activation is ENGINEERING, not a runbook)
- Activation is owner-authorized (2026-09-13, "Activate it now"). It is NOT yet mechanically landable, and it must be built and tested as Slice-3 code — NOT prescribed as a prose runbook here, because three real constraints make every naive sequence fatal and reconciling them IS the engineering: (a) `authorizedOn` (2026-09-09) is the SAME field `check-scheduled-routines.mjs` reads as the active-no-heartbeat clock, so a row flipped to `active` with no heartbeat is already past the 168h undeclared-retirement bound — the honest fix is a gate `activatedOn` baseline (`activatedOn ?? authorizedOn`), itself a `scripts/**` SAFETY_MACHINERY change; (b) `lane-deliver.mjs` writes only the heartbeat artifact in a throwaway `origin/SignalGrid_Alpha` worktree and runs the gate PRE-commit without staging `scheduled-routines.json`, while the gate FATALs on `awaiting-activation` + a fired heartbeat — so the first fire cannot land its heartbeat and flip the status in one pass; landing `active` first hits constraint (a). Activation needs a dedicated atomic transition path, not lane-deliver; (c) `create_trigger` must still come AFTER the row and a proven Slice-2 dry run (registry-first — the 2026-09-13 same-session activation was reverted for creating the trigger first, Codex #705). Until that Slice-3 code exists, the row stays `awaiting-activation`, `authorizedOn` stays the 2026-09-09 consent instant and is NEVER overwritten (the 2026-09-13 go lives in `authorizationEvidence`), and no trigger is created. `awaitingSince` is the watchdog baseline and is NOT reset to dodge the 30-day bound: if the bound is reached before activation lands, ACTIVATE, RETIRE the row with evidence, or ESCALATE — never restart the clock.
...
- **Mac liveness — RESOLVED.** `mac-lane-tick` now runs under launchd every 5 minutes on the owner's Mac (`scripts/mac/lane-tick.sh`, installed via `scripts/mac/install-launchd.sh`), heartbeating to `artifacts/agent-heartbeats/mac-lane-tick.json` (last fired 2026-09-12T09:10:32.796Z, result `quiet`). See `docs/LANE_COORDINATION.md` ("`scripts/mac/lane-tick.sh` runs every 5 minutes from launchd"). The residual risk is staleness, not absence: if the tick stops heartbeating, Swift/evidence winners go UNVERIFIED and cannot ship from cloud alone. *Mitigation:* mark them never-green and escalate staleness; cloud never blocks.
...
2. **Mac tick.** ~~Will you run `bash scripts/mac/install-launchd.sh` on your Mac? Without it, Swift/iOS/evidence audits never fire unattended.~~ **RESOLVED:** the launchd tick is installed and firing — `mac-lane-tick` runs every 5 minutes and heartbeats to `artifacts/agent-heartbeats/mac-lane-tick.json` (last fired 2026-09-12T09:10:32.796Z, result `quiet`), as the "Mac liveness — RESOLVED" risk bullet above records. The residual concern is staleness, not absence, and is handled there (mark UNVERIFIED and escalate; cloud never blocks).
