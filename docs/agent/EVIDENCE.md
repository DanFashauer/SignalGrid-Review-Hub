...
# (mtime = completion), both pass lines verbatim, and the instant CI started on the pushed head. S is the
# session scratchpad; chain logs and run-queue-<chain>.sh runners are session scratch, never tracked, so
# this record is the only place they are quoted. (Script kept as <scratchpad>/landing-evidence.sh.)
