...
| Decision class | DRI role | Consulted | Must approve (DRI cannot act alone) | Today that means |
| --- | --- | --- | --- | --- |
| **Spending money** — any recurring or one-off cost | Chief Executive Officer (founder) | Cost modeling and agent-operations economics duty, for structure only | **Founder alone** | The founder holds all spend authority and the four owner-only billing figures: model spend, developer-program fee, source-control plan, and domain spend. No owner-only billing figure (those four — DR-005 item 4) appears in this repository, and no lane estimates one — the figures stay explicit unknowns. The product's own price points and the cost model do appear (`artifacts/signalgrid-web/src/pages/Pricing.tsx`, `docs/COST_MODEL.md`); scripts/check-cost-figures.mjs, being added this round, gates the distinction. |
