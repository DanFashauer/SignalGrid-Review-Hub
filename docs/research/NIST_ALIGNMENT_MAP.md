...
runtime state, not a configuration rule; mSCP baselines *configure* a device
and verify the configuration, they do not report whether anyone has unlocked it
since boot. An earlier revision of this map put
FileVault on that row; FileVault is a macOS control modeled by the deferred
`macos-posture` family (`apple-schema.ts` → `FDE_Enabled` /
`diskmanagement.filevault.enabled`), and the row now says so. The mapping is a
positioning aid, not a claim that a live mSCP-driven posture path ships today.
...
| Compliance verification scripts (mSCP output → posture) | **None.** No mSCP output is ingested anywhere in `lib/` or `scripts/` (grep-verified for this row); an mSCP-derived posture fixture is the §1 **design target** below, proposed not built. The surfaces an assessor might mistake for it are different things: `proof:macos-posture` grades the NATIVE macOS posture collector's report (deferred, fixture-backed), the live read of that collector is the sibling **`DanFashauer/signalgrid-mcp`** Python server (verification per `docs/LIVE_SYNC_LOOP.md`), and `proof:posture-composition` / `proof:posture-allow` only combine already-normalized signals — none of the three reads an mSCP artifact | **design target** (no surface) | informed by — mSCP *reports* a device's state; SignalGrid *would decide* on it (golden rule 2: unknown posture raises assurance). Until the fixture exists this row maps a direction, not a surface |
