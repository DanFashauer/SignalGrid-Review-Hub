...
export type HandoffSimErrorCode =
  ...
  /** The current device's own composition is restrict-grade or worse. Held work
   *  is not released into the hands of a device the fabric is currently
   *  restricting — walk to a trusted device first. (Judged on the device's OWN
   *  composed action: the carried person-scoped ceiling is lowered only through
   *  its own resolution door, never by a release.) */
