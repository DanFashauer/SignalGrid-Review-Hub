  ...
  try {
    ...
    for (let depth = 0; o !== null && o !== Object.prototype; depth += 1) {
      ...
      for (const k of Reflect.ownKeys(o)) {
        ...
        // unrecognized-key check below already refuses every symbol (`includes` of a symbol
        // is always false — the PropertyKey widening below is a TYPE cast only, not a
        // behaviour change). Pinned by the prototype-walk check in the proof.
        if (!(known as readonly PropertyKey[]).includes(k)) return true;
      }
