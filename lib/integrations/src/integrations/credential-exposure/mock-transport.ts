import type { CredentialRequest, CredentialHttpResponse, CredentialTransport } from "./credential-connector";
import type { CredentialExposureRaw } from "./types";

/**
 * Deterministic in-memory stand-in for a secrets-detection platform — serves real
 ...
 * real secrets.
 */
export interface MockCredentialOptions {
  devices: CredentialExposureRaw[];
  expectedToken: string;
  pageSize?: number;
  baseUrl?: string;
}

export function createMockCredentialTransport(opts: MockCredentialOptions): CredentialTransport {
  const pageSize = Math.max(1, opts.pageSize ?? 100);
  const baseUrl = (opts.baseUrl ?? "https://api.secrets.example/v1").replace(/\/$/, "");

  return async (req: CredentialRequest): Promise<CredentialHttpResponse> => {
    ...
    if (!path.startsWith("/device-secrets")) return jsonResponse(404, { error: "not_found" });
...
function jsonResponse(status: number, body: unknown): CredentialHttpResponse {
