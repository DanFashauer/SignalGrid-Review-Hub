 ...
 * The mismatch DIRECTION is deliberately moot when matched (it only exists in
 * service of a mismatch) — the enumeration pins exactly that granting set.
 *
...
export function evaluatePolicyBinding(
  ...
  // Defence in depth: the grant is affirmative on parse, binding, match, and
  // hygiene. The branches above already push a raising candidate for every
  ...
  // matched, clean hygiene, clean parse. (Direction is moot when matched.)
