...
/// `ExpiryPolicy` removes the in-code "unknown expiry" case, but `SessionData` is
/// `Codable` and restored from the Keychain by `JSONDecoder().decode(...)` (see
/// `KeychainService.getSession`), so a malformed or tampered blob can decode
/// cleanly to `.nonExpiring(justification: "")` — the ignorance case, relocated
/// into the persistence layer. These tests pin that such a session reads as
/// EXPIRED, and would go red if a future edit re-opened the fail-open. (Row 58.)
///
...
final class SessionExpiryTests: XCTestCase {
    ...
    // MARK: the exact persistence path — decode, as KeychainService does

    /// KeychainService restores a session with `JSONDecoder().decode(SessionData…)`.
