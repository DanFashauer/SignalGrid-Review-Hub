...
// The generated provenance header that `scripts/status-summary.mjs` writes at the top of
// docs/STATUS.md — verdict, BRANCH NAME, sha, generator. A branch name is not a claim, and
// this gate read it as one: regenerating STATUS.md on a branch called
// `claude/custody-journey-v2` put the word "custody" in a block with no hedge and raised the
// docs ceiling by exactly one. Nothing in the copy had changed; the branch was named after
// the thing it builds. Left alone this recurs for every `claude/badge-*`, `claude/zone-*` or
// `claude/rtls-*` branch, and the remedies on offer were all worse than the defect — rename
// the branch, hand-edit a generated file, or hand-edit the provenance to name a branch the
// file was not generated on, which is a lie about where a status page came from.
//
// Deliberately narrow: it matches the generator's own line shape — a bolded Verdict opener
// carrying a backticked branch and `generated by \`scripts/status-summary.mjs\`` — not
// STATUS.md wholesale, and not any line that merely says "Verdict". Prose added to that
// file is still read. Self-tested in both directions below.
const GENERATED_PROVENANCE = /^\*\*Verdict:[^\n]*generated by `scripts\/status-summary\.mjs`/m;
const HEDGES =
...
function violationsIn(name, body, hedgeBody = body, commentOnlyHedges = null) {
  ...
  for (const block of pageScoped ? [] : blocksOf(name, body)) {
    ...
    if (GENERATED_PROVENANCE.test(block.text)) continue;
...
// ── self-test ────────────────────────────────────────────────────────────────
{
  ...
  const st =
    ...
    // The generated provenance header, both directions. It must be exempt when it IS the
    // generator's line, and NOT exempt for a line that merely opens with "Verdict" — the
    // exemption is for a machine-written provenance stamp, not for prose about custody.
    violationsIn("stProv0", "**Verdict: GREEN** · branch `claude/custody-journey-v2` @ `e257ea8d` · generated by `scripts/status-summary.mjs`").length === 0 &&
    violationsIn("stProv1", "**Verdict: device custody is confirmed before every session today.**").length > 0 &&
    violationsIn("stProv2", "**Verdict: GREEN** · branch `claude/custody-journey-v2` — hand-written, no generator named").length > 0 &&
