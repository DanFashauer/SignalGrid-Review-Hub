...
- **Application secrets** — any future service credentials loaded from environment variables must stay server-side and must not leak through logs, bundles, or error messages.
- **Future business and user data** — the repository already includes shared API scaffolding, so future user records, uploaded content, or business data added under `artifacts/api-server` will become high-value assets.
...
- **API to future data layer** — any future persistence layer added to the API must prevent injection and improper data exposure.
...
- Any future database access MUST use parameterized queries rather than string-built SQL.
...
Required guarantees:
- Secrets such as service credentials and future API keys MUST remain server-side only.
- Request logs MUST redact authentication material and cookies.
- Production error responses MUST avoid exposing stack traces, raw backend errors, or internal file paths.
